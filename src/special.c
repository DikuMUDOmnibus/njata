...
void summon_if_hating( CHAR_DATA * ch )
{
   ...
   if( ch->in_room == victim->in_room )
      return;
   if( !IS_NPC( victim ) )
      snprintf( buf, MAX_STRING_LENGTH, "summon 0.%s", name );
   else
...
bool spec_cast_adept( CHAR_DATA * ch )
{
   ...
   {
      case 0:
         act( AT_MAGIC, "$n utters the word 'wauxa'.", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "armor" ), ch->level, ch, victim );
         return TRUE;

      case 1:
         act( AT_MAGIC, "$n utters the word 'itlbb'.", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "bless" ), ch->level, ch, victim );
         return TRUE;

      case 2:
         act( AT_MAGIC, "$n utters the words 'sdal itpvhvlbb'.", ch, NULL, NULL, TO_ROOM );
         spell_cure_blindness( skill_lookup( "cure blindness" ), ch->level, ch, victim );
         return TRUE;

      case 3:
         act( AT_MAGIC, "$n utters the word 'bxwr'.", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "soak" ), ch->level, ch, victim );
         return TRUE;

      case 4:
         act( AT_MAGIC, "$n utters the word 'sdal yxpbxv'.", ch, NULL, NULL, TO_ROOM );
         spell_cure_poison( skill_lookup( "cure poison" ), ch->level, ch, victim );
         return TRUE;

      case 5:
         act( AT_MAGIC, "$n utters the word 'almalbo'.", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "refresh" ), ch->level, ch, victim );
         return TRUE;

      case 6:
         act( AT_MAGIC, "$n utters the word 'rvpc'.", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "knit" ), ch->level, ch, victim );
         return TRUE;

      case 7:
         act( AT_MAGIC, "$n utters the word 'aluxel sdabl'.", ch, NULL, NULL, TO_ROOM );
