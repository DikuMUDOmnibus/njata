...
ch_ret second_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt );
               ...
               switch ( attacktype )
               {
                  case DFND_CURELIGHT:
                     ...
                     if( ch->hit < ch->max_hit )
                     {
                        act( AT_MAGIC, "$n mutters a few incantations...and looks a little better.", ch, NULL, NULL,
                             TO_ROOM );
                        retcode = spell_smaug( skill_lookup( "mend" ), ch->level, ch, ch );
                     }
                     break;
                  case DFND_CURESERIOUS:
                     if( ch->hit < ch->max_hit )
                     {
                        ...
                        retcode = spell_smaug( skill_lookup( "cure" ), ch->level, ch, ch );
                     }
                     break;
                  case DFND_CURECRITICAL:
                     if( ch->hit < ch->max_hit )
                     {
                        ...
                        retcode = spell_smaug( skill_lookup( "rejuvenate" ), ch->level, ch, ch );
                     }
                     break;
                  case DFND_HEAL:
                     if( ch->hit < ch->max_hit )
...
/*  New second attack, uses second_hit -Danny */
   if( LEARNED( ch, gsn_second_attack ) > 0 )
   {
      retcode = second_hit( ch, victim, dt );
      if( retcode != rNONE || who_fighting( ch ) != victim )
         return retcode;
   }
...
/* Centaurs auto-kick, I'll probably modify this further and give it to every race in some fashion! -Danny */
   if( ch->race == RACE_CENTAUR )
   {
      schance = IS_NPC( ch ) ? ch->level : ( int )( LEARNED( ch, gsn_kick ) );
      if( number_percent(  ) < schance )
      {
...
/* No endurance loss from fighting, -Danny
...
int weapon_prof_bonus_check( CHAR_DATA * ch, OBJ_DATA * wield, int *gsn_ptr )
{
   ...
   bonus = 0;
   *gsn_ptr = gsn_hand_to_hand;   /* Change back to -1 if this fails horribly */
   if( !IS_NPC( ch ) && wield )
   {
...
ch_ret second_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   if( !wield )   /* bare hand dice formula */
      dam = number_range( ch->barenumdie, ch->baresizedie * ch->barenumdie ) + ch->damplus;
   else
      dam = number_range( wield->value[1], wield->value[2] );
...
/* Apply second attack skill to the damage */
   dam = ( int )( LEARNED( ch, gsn_second_attack ) * dam * .9 * .01 );
...
ch_ret damage( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   if( dam && dt != TYPE_UNDEFINED )
   {
      if( IS_FIRE( dt ) )
         ...
         dam = ris_damage( victim, dam, RIS_LIFE );
      else if( IS_DARKNESS( dt ) )
         dam = ris_damage( victim, dam, RIS_DARKNESS );
          ...
          */
      else if( dt == ( TYPE_HIT + DAM_CRUSH ) || gsn_kick || gsn_punch ) /* Testing this out, punch and kick are now blunt attacks */
   ...
   if( victim != ch )
   {
      ...
      /*
       * PVP stuff.  If a pvp character attacks another one or is attacked by
       * one, then ungroup any nonpvpers.  Disabled untill I can figure out
      ...
      /*
       * Check for disarm, trip, parry, block, dodge and tumble.
       */
   ...
   if( dam > 10 && dt != TYPE_UNDEFINED )
   {
       ...
       */
      dameq = number_range( WEAR_FACE, WEAR_MISSILE_WIELD );
   ...
   if( dam && ch != victim && !IS_NPC( ch ) && ch->fighting && ch->fighting->xp )
   {
      if( ch->fighting->who == victim )
         ...
         xp_gain = ( int )( xp_compute( ch, victim ) * 0.80 * dam ) / victim->max_hit;
      if( dt == gsn_backstab || dt == gsn_circle )
         xp_gain = 0;
      if( xp_gain > 0 )
         ch_printf( ch, "You gain %d experience points.\r\n", xp_gain );
      gain_exp( ch, xp_gain );
   }
   ...
   if( dam > 0 && dt > TYPE_HIT
       ...
       && is_wielding_poisoned( ch ) && !IS_SET( victim->immune, RIS_POISON ) && !saves_fortitude( ch->level, victim ) )
   {
      AFFECT_DATA af;
...
bool legal_loot( CHAR_DATA * ch, CHAR_DATA * victim )
{
    ...
    */
   if( !IS_NPC( ch ) && !IS_NPC( victim )
       && IS_SET( ch->pcdata->flags, PCFLAG_PVP ) && IS_SET( victim->pcdata->flags, PCFLAG_PVP ) )
      return TRUE;
   return FALSE;
}
...
void check_killer( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   set_char_color( AT_IMMORT, ch );
   send_to_char( "A deep voice booms inside your head, 'Thou shall now be known as a murderer!!!'\r\n", ch );
   set_char_color( AT_WHITE, ch );
...
void death_cry( CHAR_DATA * ch )
{
   ...
   if( ch->race == 34 ) /* Special death message for "constructs" - Danny */
	  msg = "$n ceases all movement and falls to the ground.";
   else switch ( number_range( 0, 5 ) )
   {
      default:
...
OBJ_DATA *raw_kill( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   OBJ_DATA *wield, *mw, *dw, *hold, *shield;

   if( !IS_NPC( victim ) )
   {
      wield = get_eq_char( victim, WEAR_WIELD );
      mw = get_eq_char( victim, WEAR_MISSILE_WIELD );
      dw = get_eq_char( victim, WEAR_DUAL_WIELD );
      ...
      {
         obj_from_char( mw );
         obj_to_room( mw, victim->in_room );
      }
      if( dw )
      {
         obj_from_char( dw );
         obj_to_room( dw, victim->in_room );
      }
      if( hold )
      {
         obj_from_char( hold );
   ...
   if( victim->race != 34 ) /* NPC Race "Construct" has no blood - Danny */
   {
...
*/
   ...
   victim->saving_fortitude = race_table[victim->race]->saving_fortitude;
   victim->saving_reflex = race_table[victim->race]->saving_reflex;
   victim->saving_will = race_table[victim->race]->saving_will;
...
/* Bounty stuff! -Danny */
   ...
   if( bounty && !IS_NPC( ch ) && !IS_NPC( victim ) && ch != victim )
...
void group_gain( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   for( gch = ch->in_room->first_person; gch; gch = gch_next )
   {
      ...
      xp = ( int )( xp_compute( gch, victim ) * 0.20 ) / members;
      if( !gch->fighting )
         xp /= 2;
...
int xp_compute( CHAR_DATA * gch, CHAR_DATA * victim )
{
   ...
   /* Still not sure what >> means in this kinda formula - Danny
   xp = number_range( ( xp * 3 ) >> 2, ( xp * 5 ) >> 2 );
    */


   /* Updated this table to be a bit more steamlined - Danny */
...
void new_dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, unsigned int dt, OBJ_DATA * obj )
{
   ...
   else if( dt > TYPE_HIT && is_wielding_poisoned( ch ) )
   {
      ...
      snprintf( buf1, 256, "$n's poisoned %s %s $N%c [%d]", attack, vp, punct, dam );
      snprintf( buf2, 256, "Your poisoned %s %s $N%c [%d]", attack, vp, punct, dam );
      snprintf( buf3, 256, "$n's poisoned %s %s you%c [%d]", attack, vp, punct, dam );
   }
   else
   {
      if( skill )
      ...
      snprintf( buf2, 256, "Your %s %s $N%c [%d]", attack, vp, punct, dam );
      snprintf( buf3, 256, "$n's %s %s you%c [%d]", attack, vp, punct, dam );
   }
...
/* Allow them to flee, even at 0 endurance -Danny
      if( ch->move <= 0 )
      {
...
*/
   ...
   else
   {
      ...
      {
	     if( !pexit->to_room || IS_SET( pexit->exit_info, EX_NOFLEE ) || ( IS_SET( pexit->exit_info, EX_CLOSED )
          && !IS_AFFECTED( ch, AFF_PASS_DOOR ) ) || ( IS_NPC( ch ) && xIS_SET( pexit->to_room->room_flags, ROOM_NO_MOB ) ) )
         {
		     act( AT_FLEE, "You can't flee in that direction!", ch, NULL, NULL, TO_CHAR );
             return;
	     }
         if( !who_fighting( ch ) )
         {
...
/* You can flee, even while out of endurance now! -Danny
         if( ch->move <= 0 )
         {
            send_to_char( "You're too exhausted to flee from combat!\r\n", ch );
            return;
