...
void fwrite_char( CHAR_DATA * ch, FILE * fp )
{
   ...
   fprintf( fp, "Sex          %d\n", ch->sex );
   fprintf( fp, "Class        %d\n", ch->Class );
   fprintf( fp, "Subclass     %d\n", ch->subclass );
   fprintf( fp, "Race         %d\n", ch->race );
   fprintf( fp, "Subrace      %d\n", ch->subrace );
   fprintf( fp, "Age          %d\n", ch->pcdata->age_bonus );
   fprintf( fp, "AgeCategory  %d\n", ch->pcdata->age_category ); /* age category - Danny */
   ...
   fprintf( fp, "Height       %d\n", ch->height );
   fprintf( fp, "Weight       %d\n", ch->weight );
   if( ch->pcdata->hair && ch->pcdata->hair[0] != '\0' ) /* hair description - Danny */
      fprintf( fp, "Hair         %s~\n", ch->pcdata->hair );
   if( ch->pcdata->eyes && ch->pcdata->eyes[0] != '\0' ) /* eyes description - Danny */
      fprintf( fp, "Eyes         %s~\n", ch->pcdata->eyes );
   fprintf( fp, "BodyShape    %d\n", ch->bodyshape );
   ...
   fprintf( fp, "SavingThrows %d %d %d\n", ch->saving_fortitude, ch->saving_reflex, ch->saving_will );
   ...
   if( ch->pcdata->email && ch->pcdata->email[0] != '\0' ) /* Samson 4-19-98 */
      fprintf( fp, "Email        %s~\n", ch->pcdata->email );
   if( ch->pcdata->aim && ch->pcdata->aim[0] != '\0' ) /* Danny */
      fprintf( fp, "AIM          %s~\n", ch->pcdata->aim );
...
bool load_char_obj( DESCRIPTOR_DATA * d, const char *name, bool preload, bool copyover )
{
   ...
   ch->pcdata->aim = NULL; /* Initialize AIM -Danny */
   ch->pcdata->hair = NULL; /* Initialize hair description -Danny */
   ch->pcdata->eyes = NULL; /* Initialize eyes description -Danny */
   ...
   if( !found )
   {
      ...
      /* every characters starts at default board from login.. this board should be read_level == 0 ! */
      ...
      ch->pcdata->aim = str_dup( "" ); /* Danny */
      ch->pcdata->hair = str_dup( "" ); /* Danny */
      ch->pcdata->eyes = str_dup( "" ); /* Danny */
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'A':
            ...
            KEY( "AgeCategory", ch->pcdata->age_category, fread_number( fp ) );
         ...
         case 'B':
            ...
            KEY( "BodyShape", ch->bodyshape, fread_number( fp ) );
            break;
         ...
         case 'H':
	    	KEY( "Hair", ch->pcdata->hair, fread_string_nohash( fp ) );
         ...
         case 'S':
            ...
            KEY( "Subclass", ch->subclass, fread_number( fp ) );
            KEY( "Subrace", ch->subrace, fread_number( fp ) );
            KEY( "Susceptible", ch->susceptible, fread_number( fp ) );
            if( !strcmp( word, "SavingThrow" ) )
            {
               ch->saving_fortitude = fread_number( fp );
               ch->saving_reflex = ch->saving_fortitude;
               ch->saving_will = ch->saving_fortitude;
               fMatch = TRUE;
               break;
            }
            ...
            {
               ch->saving_fortitude = fread_number( fp );
               ch->saving_reflex = fread_number( fp );
               ch->saving_will = fread_number( fp );
               fMatch = TRUE;
               break;
            }
         ...
         case 'E':
	    	...
	    	KEY( "Eyes", ch->pcdata->eyes, fread_string_nohash( fp ) );
            break;
...
void do_last(CHAR_DATA * ch, const char *argument)
{
   ...
   if (isdigit(arg[0]) || atoi(arg) == -1 || !str_cmp(arg, "today")) //View list instead of players
   {
      ...
      if (!str_cmp(arg, "today"))
         read_last_file(ch, -2, NULL, 0);
      else
         read_last_file(ch, atoi(arg), NULL, 0);
      return;
   }
   ...
   {
      send_to_char("&w&RName                     Time                        Host/Ip\n\r&c&w---------------------------------------------------------------------------\n\r", ch);
      read_last_file(ch, atoi(argument), name, 0);
      return;
   }
