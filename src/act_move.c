...
const char *const sect_names[SECT_MAX][2] = {
   ...
   {"In a desert", "deserts"}, {"In the ocean", "waters"},
...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
   ...
   if( pexit )
   {
      log_printf( "move_char: %s to door %d", ch->name, pexit->vdir );
   }
#endif
    ...
    */
   if( ch->position == POS_MOUNTED && !IS_NPC( ch->mount ) )
   {
	  send_to_char( "Your mount hasn't moved yet, try dismounting if you want to go that way.\r\n", ch );
	  return rNONE;
   }
   ...
   if( !fall && !IS_NPC( ch ) )
   {
          ...
          && !IS_SET( ch->in_room->area->flags, AFLAG_NOPKILL ) && ( IS_PKILL( ch ) && !IS_IMMORTAL( ch ) ) )
      {
         ...
         send_to_char( "\r\nA godly force forbids PVP characters from entering that area...\r\n", ch );
         return rNONE;
      }
...
/*  TINY exit flag = need to be a foot or smaller to fit -Danny */
      if( IS_SET( pexit->exit_info, EX_TINY ) || xIS_SET( to_room->room_flags, ROOM_TINY ) )
      {
         if( ch->mount && ch->mount->height > 12 )
         {
            send_to_char( "Your mount is too large to fit.\r\n", ch );
            return rNONE;
         }
         if( !ch->mount && ch->height > 12 )
         {
            send_to_char( "You're too large to fit!\r\n", ch );
            return rNONE;
         }
      }
/*  Can't fly into an underwater square -Danny */
      if( to_room->sector_type == SECT_UNDERWATER )
      {
         ...
         {
            send_to_char( "Your mount can't fly underwater!\r\n", ch );
            return rNONE;
         }
      }
      ...
      else
      {
         if( !IS_FLOATING( ch ) )
         {
		    ...
		    if( ch->race == RACE_FAIRY )
		       move *= 2;
		    else if( ch->race == RACE_MERFOLK && !xIS_SET( ch->act, PLR_HASLEGS ) )
		    {
		       if( in_room->sector_type == SECT_WATER_SWIM || in_room->sector_type == SECT_OCEAN )
		          move = 1;
		       else
		          move *= 3;
            }
      ...
      if( ( IS_SET( pexit->exit_info, EX_CLOSED ) ) && ( IS_AFFECTED( ch, AFF_PASS_DOOR ) )
   		           && ( ch->race == RACE_SLIME ) && ( !IS_SET( pexit->exit_info, EX_NOPASSDOOR ) ) )
      {
         act( AT_PLAIN, "$n seeps through the $d.", ch, NULL, pexit->keyword, TO_ROOM );
         act( AT_PLAIN, "You seep through the $d.", ch, NULL, pexit->keyword, TO_CHAR );
      }
      if( ch->mount )
         ch->mount->move -= move;
      else
   ...
   if( !IS_AFFECTED( ch, AFF_SNEAK ) && ( IS_NPC( ch ) || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
   {
      ...
      else if( !txt )
      {
         ...
         else
         {
            ...
            else if( ( ch->race == RACE_CATFOLK ) || ( ch->race == RACE_WOLFFOLK )
            	|| ( ch->race == RACE_SPHINX ) || (ch->race == RACE_GRYPHON ) )
               txt = "pads";
            else if( ch->race == RACE_FIREBIRD )
               txt = "swaggers";
            else if( ( ch->race == RACE_DRYAD ) || ( ch->race == RACE_NAIAD ) )
               txt = "floats";
            else if( ch->race == RACE_DOLLFOLK )
               txt = "clanks";
            else if( ch->race == RACE_SLIME )
               txt = "slides";
            else if( ch->race == RACE_MERFOLK && !xIS_SET( ch->act, PLR_HASLEGS ) )
               txt = "crawls";
      ...
      {
         snprintf( buf, MAX_STRING_LENGTH, "$n %s %s with $N on $s back.", txt, dir_name[door] );
         act( AT_ACTION, buf, ch, NULL, ch->rider, TO_NOTVICT );
         snprintf( buf, MAX_STRING_LENGTH, "$n %s %s with you on $s back.", txt, dir_name[door] );
         act( AT_ACTION, buf, ch, NULL, ch->rider, TO_VICT );
      }
      else
      {
   ...
   if( !IS_AFFECTED( ch, AFF_SNEAK ) && ( IS_NPC( ch ) || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
   {
      ...
      {
         snprintf( buf, MAX_STRING_LENGTH, "$n %s from %s with $N on $s back.", txt, dtxt );
         act( AT_ACTION, buf, ch, NULL, ch->rider, TO_NOTVICT );
      }
      else
      {
...
/*New do_stand that allows for interaction with furniture -Danny */
void do_stand( CHAR_DATA * ch, const char *argument )
{
   ...
   /*
    * okay, now that we know we can stand, find an object to stand on
    */
   if( argument[0] != '\0' )
   {
...
/* New do_sit allows you to sit on furniture items -Danny */
void do_sit( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      send_to_char( "You're currently flying. Perhaps you should land first?\n\r", ch );
      return;
   }
   /*
