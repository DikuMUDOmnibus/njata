...
void show_char_to_char_squint( CHAR_DATA * victim, CHAR_DATA * ch );
...
const char *get_skill_rank( short percentage )
{
   if( percentage == 100 )
      return ( "Paramount" );
...
//         send_to_char_color( "&P(&WImmortal&P) ", ch );
      if( victim->pcdata->clan
          && IS_SET( victim->pcdata->flags, PCFLAG_PVP )
...
void show_char_to_char_squint( CHAR_DATA * victim, CHAR_DATA * ch )
{
   ...
   {
      act( AT_ACTION, "$n squints at you.", ch, NULL, victim, TO_VICT );
      if( victim != ch )
         act( AT_ACTION, "$n squints at $N.", ch, NULL, victim, TO_NOTVICT );
      else
         act( AT_ACTION, "$n squints at $mself.", ch, NULL, victim, TO_NOTVICT );
   }
   ...
   else
   {
      if( victim->morph != NULL && victim->morph->morph != NULL )
         ...
         act( AT_PLAIN, "$E is a rather nondescript person.", ch, NULL, victim, TO_CHAR );
      else
         act( AT_PLAIN, "You're a rather nondescript person.", ch, NULL, NULL, TO_CHAR );
   }
   ...
   if( !IS_NPC( victim ) )
   {
      if( ch != victim )
      {
         if( victim->pcdata->hair && victim->pcdata->hair[0] != '\0' )
            act( AT_PLAIN, "$E has $t.", ch, victim->pcdata->hair, victim, TO_CHAR );
         if( victim->pcdata->eyes && victim->pcdata->eyes[0] != '\0' )
            act( AT_PLAIN, "$E has $t.", ch, victim->pcdata->eyes, victim, TO_CHAR );
      }
      else
      {
         if( ch->pcdata->hair && ch->pcdata->hair[0] != '\0' )
            act( AT_PLAIN, "You have $t.", ch, ch->pcdata->hair, NULL, TO_CHAR );
         if( ch->pcdata->eyes && ch->pcdata->eyes[0] != '\0' )
            act( AT_PLAIN, "You have $t.", ch, ch->pcdata->eyes, NULL, TO_CHAR );
      }
   }
   show_body_line( ch, victim );
...
void do_squint( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !IS_NPC( ch )
       && !xIS_SET( ch->act, PLR_HOLYLIGHT ) && !IS_AFFECTED( ch, AFF_TRUESIGHT ) && !IS_AFFECTED( ch, AFF_DARKVISION ) && room_is_dark( ch->in_room ) )
   {
   ...
   {
      act( AT_ACTION, "You squint at the $d.", ch, NULL, arg1, TO_CHAR );
      act( AT_ACTION, "$n squints at the $d.", ch, NULL, arg1, TO_ROOM );
   ...
   {
      show_char_to_char_squint( victim, ch );
      return;
   }
...
void do_look( CHAR_DATA * ch, const char *argument )
{
   ...
   if( arg1[0] == '\0' || !str_cmp( arg1, "auto" ) )
   {
      ...
      {
         /* Is it winter? Then use the winter_desc if there is one (temporarily removed to change to desc_cold */
/*         if( ( time_info.season == SEASON_WINTER ) && ( pdesc = get_extra_descr( "desc_winter", ch->in_room->first_extradesc ) ) ) */
         /* New plan, if the temperature is below 30 in a room, show the "desc_freezing" ed instead */
         cell = getWeatherCell( ch->in_room->area );
         if( ( getTemp( cell ) <= 30 ) && ( pdesc = get_extra_descr( "desc_freezing", ch->in_room->first_extradesc ) ) )
   	     {
            if( xIS_SET( ch->act, PLR_AUTOMAP ) )
               draw_room_map( ch, pdesc );
	        else
	           send_to_char( pdesc, ch);
	     }
         else if( xIS_SET( ch->act, PLR_AUTOMAP ) )
   ...
   if( ( pdesc = get_extra_descr( arg1, ch->in_room->first_extradesc ) ) != NULL )
   {
      act( AT_ACTION, "You look at the $d.", ch, NULL, arg1, TO_CHAR );
      act( AT_ACTION, "$n looks at the $d.", ch, NULL, arg1, TO_ROOM );
      send_to_char_color( pdesc, ch );
      return;
   }
...
void show_race_line( CHAR_DATA * ch, CHAR_DATA * victim )
{
   if( !IS_NPC( victim ) && !IS_NPC( ch ) )
   {
      if( ch->pcdata->age_category == AGE_CHILD )
      {
         if( victim != ch )
         {
            ch_printf( ch, "You see a %s %s child.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                         get_race( victim ) );
            return;
         }
         if( victim == ch )
         {
            ch_printf( ch, "You are a %s %s child.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                         get_race( victim ) );
            return;
         }
      }
      if( ch->pcdata->age_category == AGE_YOUTH )
      {
         if( victim != ch )
         {
            ch_printf( ch, "You see a %s %s youth.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                         race_table[victim->race]->race_name );
            return;
         }
         if( victim == ch )
         {
            ch_printf( ch, "You are a %s %s youth.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                         race_table[victim->race]->race_name );
            return;
         }
      }
      if( ch->pcdata->age_category == AGE_ADULT )
      {
         if( victim != ch )
         {
            ch_printf( ch, "You see an adult %s %s.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                          race_table[victim->race]->race_name );
            return;
         }
         if( victim == ch )
         {
            ch_printf( ch, "You are an adult %s %s.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                          race_table[victim->race]->race_name );
            return;
         }
      }
      if( ch->pcdata->age_category == AGE_MIDDLE )
      {
         if( victim != ch )
         {
            ch_printf( ch, "You see a middle-aged %s %s.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                               race_table[victim->race]->race_name );
            return;
         }
         if( victim == ch )
         {
            ch_printf( ch, "You are a middle-aged %s %s.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                               race_table[victim->race]->race_name );
            return;
         }
      }
      if( ch->pcdata->age_category == AGE_ELDERLY )
      {
         if( victim != ch )
         {
            ch_printf( ch, "You see an elderly %s %s.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                                  race_table[victim->race]->race_name );
            return;
         }
         if( victim == ch )
         {
            ch_printf( ch, "You are an elderly %s %s.\r\n", victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",
                                                                  race_table[victim->race]->race_name );
            return;
         }
      }
...
void show_body_line( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( !IS_NPC( victim ) && ( victim != ch ) )
   {
      ...
      /* Shows height and bodyshape to viewer */
      if( feet > 0 )
         ch_printf( ch, "%s is %d'%d\" with a %s body.\r\n", PERS( victim, ch ), feet, inches, body_shape[victim->bodyshape] );
      else
         ch_printf( ch, "%s is %d inches tall with a %s body.\r\n", PERS( victim, ch ), inches, body_shape[victim->bodyshape] );
      return;
   }
   if( !IS_NPC( victim ) && ( victim == ch ) )
   {
      feet = victim->height / 12;
      inches = victim->height % 12;
      /* Shows height and bodyshape to yourself */
      if( feet > 0 )
         ch_printf( ch, "You are %d'%d\" with a %s body.\r\n", feet, inches, body_shape[victim->bodyshape] );
      else
         ch_printf( ch, "You are %d inches tall with a %s body.\r\n", inches, body_shape[victim->bodyshape] );
      return;
   }
}
      ...
      switch ( obj->item_type )
      {
         ...
         case ITEM_FOOD:
            if( obj->timer > 0 && obj->value[1] > 0 )
               ...
               dam = 10;
            if( ( obj->item_type == ITEM_FOOD ) || ( obj->item_type == ITEM_MEAT )
             || ( obj->item_type == ITEM_FISH ) || ( obj->item_type == ITEM_PLANT ) )
               mudstrlcpy( buf, "As you examine it carefully you notice that it ", MAX_STRING_LENGTH );
            else
...
void do_exits( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !IS_NPC( ch ) && !xIS_SET( ch->act, PLR_HOLYLIGHT ) && !IS_AFFECTED( ch, AFF_TRUESIGHT )
   && !IS_AFFECTED( ch, AFF_DARKVISION ) && room_is_dark( ch->in_room ) )
   {
      set_char_color( AT_DGREY, ch );
      send_to_char( "It is pitch black ... \r\n", ch );
      return;
   ...
   for( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
   {
      ...
      /* If it's a closed door and hidden/secret/diggable, don't show */
      if( IS_SET( pexit->exit_info, EX_CLOSED ) && ( IS_SET( pexit->exit_info, EX_HIDDEN ) || IS_SET( pexit->exit_info, EX_SECRET ) || IS_SET( pexit->exit_info, EX_DIG ) ) )
         continue;

      /* Not a door, but still hidden/secret, don't show */
      if( !IS_SET( pexit->exit_info, EX_ISDOOR ) && ( IS_SET( pexit->exit_info, EX_HIDDEN ) || IS_SET( pexit->exit_info, EX_SECRET ) ) )
         continue;
...
void do_exits( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !IS_NPC( ch )
       ...
       && !IS_AFFECTED( ch, AFF_TRUESIGHT ) && !IS_AFFECTED( ch, AFF_INFRARED ) && !IS_AFFECTED( ch, AFF_DARKVISION ) && room_is_dark( ch->in_room ) )
   {
      set_char_color( AT_DGREY, ch );
      send_to_char( "It is pitch black ... \r\n", ch );
      return;
...
void do_help( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      pager_printf( ch, "&wNo help on \'%s\' found.\r\n", argument );
      /*
...
void do_who( CHAR_DATA* ch, const char* argument)
{
   ...
   for( ;; )
   {
      ...
      else
      {
          ...
          */
         if( !str_cmp( arg, "pvp" ) || !str_cmp( arg, "pkill" ) )
            fPkill = TRUE;
...
/* start from last to first to get it in the proper order */
        ...
        d = ( fGroup ? ( whogr ? whogr->d : NULL ) : d->prev ) )
   {
      ...
      /* Completely ignore everything above this and set Class to Race anyway... -Danny */
      else
         Class = capitalize( race_table[wch->race]->race_name );
      if( wch->pcdata->clan )
      {
      ...
      if( xIS_SET( wch->act, PLR_WIZINVIS ) )
         ...
         invis_str[0] = '\0';
      snprintf( buf, MAX_STRING_LENGTH, "&w%*s%-16s %s%s%s%s%s%s%s%s%s%s%s&w\r\n",
   ...
   for( cur_who = first_pvp; cur_who; cur_who = next_who )
   {
      if( !ch )
         fprintf( whoout, "%s", cur_who->text );
      else
...
/* For players who want to practice, direct them to our new command -Danny */
void do_practice( CHAR_DATA* ch, const char* argument)
{
   send_to_char( "I'm not sure what you mean. Perhaps you meant to check your [slist]?\r\n", ch );
   return;
}

/* New learn skill, functions nearly identically to practice -Danny */
void do_learn( CHAR_DATA* ch, const char* argument)
{
   ...
   else
   {
      ...
      if( skill->req_skill && skill->req_skill != -1 && ch->pcdata->learned[ skill->req_skill ] < RANK_PROFESSIONAL )
      {
         act( AT_TELL, "$n tells you 'You don't have the skill required to learn.'", mob, NULL, ch, TO_VICT );
         return;
      }
      ...
      {
         snprintf( buf, MAX_STRING_LENGTH, "$n tells you, 'I've taught you everything I can about %s.'", skill->name );
         act( AT_TELL, buf, mob, NULL, ch, TO_VICT );
         act( AT_TELL, "$n tells you, 'You'll have to practice it on your own now...'", mob, NULL, ch, TO_VICT );
      }
      else
      {
/* Characters learn skills at 1% (beginning of dilettante) -Danny */
         ch->pcdata->learned[sn] = 1;
         act( AT_ACTION, "You learn $T.", ch, NULL, skill->name, TO_CHAR );
         act( AT_ACTION, "$n learns $T.", ch, NULL, skill->name, TO_ROOM );
         if( ch->pcdata->learned[sn] >= 1 )
         {
            ch->pcdata->learned[sn] = 1;
            act( AT_TELL, "$n tells you. 'You'll have to practice it on your own now...'", mob, NULL, ch, TO_VICT );
...
/* Spells command to just view spells that you have learned! -Danny */
void do_spells( CHAR_DATA* ch, const char* argument)
{
   ...
   for( sn = 0; sn < num_skills; ++sn )
   {
      ...
      if( strcmp( skill->name, "acid blast" ) == 0 )
         send_to_pager_color( "&C---------------------[&WSpells&C]---------------------\r\n", ch );
...
/*  Doesn't show skills with 0% learning -Danny */
      if( ch->pcdata->learned[skill_lookup( skill->name )] <= 0 )
         continue;
      ...
      ++cnt;
      set_pager_color( AT_MAGIC, ch );
      pager_printf( ch, "    &C%-30s", skill->name );
      if( ch->pcdata->learned[normalSn] == 100 )
         pager_printf( ch, " &WParamount ");
      else if ( ch->pcdata->learned[normalSn] >= 89 )
         pager_printf( ch, " &WVirtuoso ");
      else if ( ch->pcdata->learned[normalSn] >= 78 )
         pager_printf( ch, " &WExpert ");
      else if ( ch->pcdata->learned[normalSn] >= 67 )
         pager_printf( ch, " &WSavant ");
      else if ( ch->pcdata->learned[normalSn] >= 56 )
         pager_printf( ch, " &WProfessional ");
      else if ( ch->pcdata->learned[normalSn] >= 45 )
         pager_printf( ch, " &WAdept ");
      else if ( ch->pcdata->learned[normalSn] >= 34 )
         pager_printf( ch, " &WArtisan ");
      else if ( ch->pcdata->learned[normalSn] >= 23 )
         pager_printf( ch, " &WPractitioner ");
      else if ( ch->pcdata->learned[normalSn] >= 12 )
         pager_printf( ch, " &WDisciple ");
      else
         pager_printf( ch, " &WDilettante ");
      send_to_pager( "\r\n", ch );
   }

   if( !cnt )
      send_to_pager( "                       &C(&Wnone&C)\r\n", ch );
   if( col % 3 != 0 )
      send_to_pager( "\r\n", ch );
   return;
}
...
void do_skills( CHAR_DATA* ch, const char* argument)
{
   ...
   for( sn = 0; sn < num_skills; ++sn )
   {
      ...
      if( strcmp( skill->name, "acid blast" ) == 0 )
         send_to_pager_color( "&C---------------------[&WSkills&C]---------------------\r\n", ch );
...
/*  Doesn't show secretskills unless you've already learned them -Danny */
      if( ch->pcdata->learned[skill_lookup( skill->name )] <= 0 && SPELL_FLAG( skill, SF_SECRETSKILL ) )
         continue;
...
/*  Doesn't show skills unless they're available at your level on race/class list -Danny */
      is_ok = FALSE;
      if( ch->level >= skill->skill_level[ch->Class] )
         is_ok = TRUE;
      if( ch->level >= skill->race_level[ch->race] )
...
void do_proficiencies( CHAR_DATA* ch, const char* argument)
{
   int sn;
   ...
   for( sn = 0; sn < num_skills; ++sn )
   {
      ...
      if( strcmp( skill->name, "acid blast" ) == 0 )
         send_to_pager_color( "&C------------------[&WProficiencies&C]-----------------\r\n", ch );

      if( skill->type != SKILL_WEAPON )
         continue;
   ...
   for( sn = 0; sn < num_skills; ++sn )
   {
      ...
      if( strcmp( skill->name, "acid blast" ) == 0 )
      {
         if( col % 3 != 0 )
            send_to_pager( "\r\n", ch );
         set_pager_color( AT_MAGIC, ch );
         send_to_pager_color( "&C--------------------[&WLanguages&C]-------------------\r\n",
                              ch );
         col = 0;
      }
...
void do_train( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( !str_cmp( argument, "str" ) )
      ...
      {
        AttrPerm = &ch->max_hit;
        pOutput = (char *)"number of hit points";
...
void do_password( CHAR_DATA* ch, const char* argument)
{
   ...
   *pArg = '\0';
   ...
   if( ch->desc && ch->desc->host[0] != '\0' )
      ...
      log_printf( "%s changing their password with no descriptor!", ch->name );
   send_to_char( "Ok.\r\n", ch );
}
...
void do_channels( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg[0] == '\0' )
   {
      ...
      ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_PVP ) ? " &G+PVP" : " &g-pvp" );
      ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_LFG ) ? " &G+LFG" : " &g-lfg" );
   ...
   else
   {
      ...
      if( ( fClear ) && ( ClearAll ) )
      {
         ...
         REMOVE_BIT( ch->deaf, CHANNEL_PVP );
         REMOVE_BIT( ch->deaf, CHANNEL_PRAY );
         REMOVE_BIT( ch->deaf, CHANNEL_LFG );
...
void do_config( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg[0] == '\0' )
   {
      ...
      ch_printf( ch, "%-12s   %-12s   %-12s   %-12s",
                 ...
                 xIS_SET( ch->act, PLR_RESETMSG ) ? "[+] RESETMSG" : "[-] resetmsg" );
   ...
   else
   {
      ...
      if( !str_prefix( arg + 1, "autoexit" ) )
         ...
         bit = PLR_AUTOMAP;
      else if( !str_prefix( arg + 1, "resetmsg" ) )
         bit = PLR_RESETMSG;
/*	else if ( !str_prefix( arg+1, "flee"     ) ) bit = PLR_FLEE; */
      ...
      if( bit )
      {
         if( ( bit == PLR_FLEE || bit == PLR_NICE || bit == PLR_SHOVEDRAG ) && IS_SET( ch->pcdata->flags, PCFLAG_PVP ) )
         {
            send_to_char( "PVP characters can not config that option.\r\n", ch );
            return;
         }
...
void do_slist( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      lFound = 0;
      snprintf( skn, MAX_INPUT_LENGTH, "&C%s", "&CSpell" );
      for( sn = 0; sn < num_skills; ++sn )
      {
         const SKILLTYPE *skill;
         int normalSn;
         ...
         if( i == skill->skill_level[ch->Class] || i == skill->race_level[ch->race] )
         {
            ...
            {
               lFound = 1;
               pager_printf( ch, "&CLevel &W%d\r\n", i );
            }

            if( skill->req_skill && skill->req_skill != -1 )
               pager_printf( ch, "&C%7s: &W%20.20s \t&CCurrent: &W%-15s&CRequired: &W%s \r\n",
                          skn, skill->name, get_skill_rank( ch->pcdata->learned[normalSn] ), skill_table[ skill->req_skill ]->name );
            else
               pager_printf( ch, "&C%7s: &W%20.20s \t&CCurrent: &W%-15s&CRequired: &W%s \r\n",
                          skn, skill->name, get_skill_rank( ch->pcdata->learned[normalSn] ), "None" );
         }
      }
   }
   return;
...
void do_whois( CHAR_DATA* ch, const char* argument)
{
   ...
   pager_printf( ch, " %s is a %spvp player",
                 victim->sex == SEX_MALE ? "He" :
                 victim->sex == SEX_FEMALE ? "She" : "It", IS_SET( victim->pcdata->flags, PCFLAG_PVP ) ? "" : "non-" );
   ...
   if( IS_IMMORTAL( ch ) )
   {
      ...
      {
         snprintf( buf2, MAX_STRING_LENGTH, "%s", "This player has the following flags set:" );
...
void do_ignore( CHAR_DATA* ch, const char* argument)
{
    ...
    */
   else if( !strcmp( arg, "self" ) || nifty_is_name( arg, ch->name ) || !strcmp( arg, "me" ) )
   {
      set_char_color( AT_IGNORE, ch );
