...
/*
 * Calculate how much experience a character is worth based on level
 */

int get_exp_worth_by_level( CHAR_DATA * ch, int level)
{
	int exp;
	exp = (get_exp_worth( ch ) * URANGE( 0, (ch->level - level) + 10, 13 )) / 10;
...
int can_carry_n( CHAR_DATA * ch )
{
   ...
   if( get_eq_char( ch, WEAR_SHIELD ) )
      ++penalty;
   return URANGE( 5, ( ch->level + 15 ) / 5 + get_curr_dex( ch ) - 10 - penalty, 20 );
}
...
void modify_skill( CHAR_DATA * ch, int sn, int mod, bool fAdd )
{
   if( !IS_NPC( ch ) )
   {
      if( fAdd )
         ...
         ch->pcdata->learned[sn] = URANGE( 0, ch->pcdata->learned[sn] + mod, RANK_PARAMOUNT );
   }
}
...
void affect_modify( CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd )
{
   ...
   switch ( paf->location % REVERSE_APPLY )
   {
      ...
      case APPLY_SAVING_FORTITUDE:
         ch->saving_fortitude += mod;
         break;
      case APPLY_SAVING_REFLEX:
         ch->saving_reflex += mod;
         break;
...
void char_from_room( CHAR_DATA * ch )
{
   ...
   for( obj = ch->first_carrying; obj != NULL; obj = obj_next )
   {
      ...
      if( obj->wear_loc != WEAR_NONE && can_see_obj( ch, obj ) )
         if( ( obj = get_eq_char( ch, obj->wear_loc ) ) != NULL && obj->item_type == ITEM_LIGHT && obj->value[2] != 0 && ch->in_room->light > 0 )
            --ch->in_room->light;
   }
...
void char_to_room( CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex )
{
   ...
   for( obj = ch->first_carrying; obj != NULL; obj = obj_next )
   {
      obj_next = obj->next_content;
      if( obj->wear_loc != WEAR_NONE && can_see_obj( ch, obj ) )
         if( ( obj = get_eq_char( ch, obj->wear_loc ) ) != NULL && obj->item_type == ITEM_LIGHT && obj->value[2] != 0 )
            ++pRoomIndex->light;
   }
...
/* Tail slot takes up legs AND feet -Danny */
      case WEAR_TAIL:
         return 3 * obj->value[0];
      case WEAR_HEAD:
      ...
      case WEAR_BACK:
         return obj->value[0];
      case WEAR_FORE_ANKLES:
         return obj->value[0];
      case WEAR_HIND_ANKLES:
         return obj->value[0];
      case WEAR_FORE_LEGS:
         return obj->value[0];
      case WEAR_HIND_LEGS:
...
OBJ_DATA *get_eq_hold( CHAR_DATA *ch, int type )
{
   ...
   for( obj = ch->first_carrying; obj; obj = obj->next_content )
   {
      if( ( obj->wear_loc == WEAR_HOLD ) && obj->item_type == type )
      {
         if( !obj->pIndexData->layers )
            return obj;
         else if( !maxobj || obj->pIndexData->layers > maxobj->pIndexData->layers )
...
/* Items heavier than 4 will slide right through a Slime character -Danny */
   if( ch->race == RACE_SLIME && obj->weight > 4 )
   {
      if( loading_char != ch )
      {
         act( AT_ACTION, "$p is too heavy and slides right through you.", ch, obj, NULL, TO_CHAR );
         act( AT_ACTION, "$p is too heavy and slides right through $n.", ch, obj, NULL, TO_ROOM );
      }
      if( obj->carried_by )
         obj_from_char( obj );
      obj_to_room( obj, ch->in_room );
...
/* Players keep their items through death!
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
   ...
   if( !IS_AFFECTED( ch, AFF_TRUESIGHT ) )
   {
      ...
      if( room_is_dark( ch->in_room ) && !IS_AFFECTED( ch, AFF_INFRARED ) && !IS_AFFECTED( ch, AFF_DARKVISION ) )
         return FALSE;
...
bool can_see_obj( CHAR_DATA * ch, OBJ_DATA * obj )
{
   ...
   if( room_is_dark( ch->in_room ) )
   {
      ...
      if( IS_OBJ_STAT( obj, ITEM_GLOW ) )
         return TRUE;
      if( !IS_AFFECTED( ch, AFF_INFRARED ) && !IS_AFFECTED( ch, AFF_DARKVISION ) )
         return FALSE;
   }
...
const char *affect_bit_name( EXT_BV * vector )
{
   ...
   if( xIS_SET( *vector, AFF_DARKVISION ) )
      mudstrlcat( buf, " darkvision", 512 );
...
bool can_astral( CHAR_DATA * ch, CHAR_DATA * victim )
{
   if( victim == ch
       ...
       || ( IS_NPC( victim ) && saves_will( ch->level, victim ) )
 ...
 */
   ...
   if( ( number_percent(  ) - get_curr_lck( ch ) + 10 ) <= percent )
      return TRUE;
   else
      return FALSE;
}
