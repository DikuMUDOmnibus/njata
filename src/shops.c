...
int get_cost( CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj, bool fBuy )
{
   ...
   if( fBuy )
   {
      profitmod = 10 - get_curr_cha( ch ) + ( richcustomer ? 15 : 0 )
   ...
   else
   {
      ...
      profitmod = get_curr_cha( ch ) - 10 - ( richcustomer ? 15 : 0 );
      cost = 0;
...
void do_buy( CHAR_DATA* ch, const char* argument)
{
   ...
   if( xIS_SET( ch->in_room->room_flags, ROOM_PET_SHOP ) )
   {
      ...
      if( !IS_IMMORTAL( ch ) && xIS_SET( ch->act, PLR_BOUGHT_PET ) )
      {
   ...
   else
   {
      ...
      discount = 0;
      if( !IS_NPC( ch ) && LEARNED( ch, gsn_haggle ) > 0 )
      {
         discount = ( cost * LEARNED( ch, gsn_haggle ) / 1000 );
         learn_from_success( ch, gsn_haggle );
      }
      ...
      ch->gold -= ( cost - discount );
      keeper->gold += ( cost - discount );
...
/*      bool listall; */
      ...
      for( obj = keeper->first_carrying; obj; obj = obj->next_content )
      {
             ...
             && ( cost = get_cost( ch, keeper, obj, TRUE ) ) > 0 && ( arg[0] == '\0' || nifty_is_name( arg, obj->name ) ) )
         {
            ...
            pager_printf( ch, "[%2d %5d] %s.\r\n", obj->level, ( cost - discount ), capitalize( obj->short_descr ) );
         }
      }
...
void do_sell( CHAR_DATA* ch, const char* argument)
{
   ...
   snprintf( buf, MAX_STRING_LENGTH, "You sell $p for %d gold piece%s.", ( cost + discount ), ( cost + discount ) == 1 ? "" : "s" );
...
void do_value( CHAR_DATA* ch, const char* argument)
{
   ...
   snprintf( buf, MAX_STRING_LENGTH, "$n tells you 'I'll give you %d gold coins for $p.'", ( cost + discount ) );
