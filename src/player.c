...
/*
 * Old score command, kept in for NPC use.
 */
...
void do_newscore( CHAR_DATA* ch, const char* argument )
{
    ...
    pager_printf_color(ch, "&C%s%s.\n\r", ch->name, ch->pcdata->title);
    if ( get_trust( ch ) != ch->level )
	pager_printf( ch, "You are trusted at level %d.\n\r", get_trust( ch ) );

    send_to_pager_color("&W-------------------------------------------------------------------------------\n\r", ch);

    pager_printf_color(ch, "&CLevel: &W%-3d        &CRace : &W%-10.10s            &CPlayed: &W%ld &Chours\n\r",
	ch->level, capitalize(get_race(ch)), ( long int )GET_TIME_PLAYED( ch ) );
    pager_printf_color(ch, "&CYears: &W%-6d     &CClass: &W%-12.12s          &CLog In: %s\r",
		calculate_age( ch ), capitalize(get_class(ch)), ctime(&(ch->logon)) );

	pager_printf_color(ch, "&CSTR  : &W%2.2d&C(&w%2.2d&C)  &CSubclass: &W%-23s&CSaved: %s\r",
		get_curr_str(ch), ch->perm_str, get_subclass(ch->subclass), ch->save_time ? ctime(&(ch->save_time)) : "no save this session\n" );

	pager_printf_color(ch, "&CDEX  : &W%2.2d&C(&w%2.2d&C)                                    &CTime: %s\r",
		get_curr_dex(ch), ch->perm_dex, ctime(&current_time) );
	...
	pager_printf_color(ch, "&CCON  : &W%2.2d&C(&w%2.2d&C)     Armor: &W%s \n\r",
		get_curr_con(ch), ch->perm_con, buf);

	pager_printf_color(ch, "&CINT  : &W%2.2d&C(&w%2.2d&C)                                   &CItems:  &W%d &w(max %d)\n\r",
		get_curr_int( ch ), ch->perm_int, ch->carry_number, can_carry_n( ch ) );
    ...
    pager_printf_color(ch, "&CWIS  : &W%2.2d&C(&w%2.2d&C)     Pos'n: &W%-21.21s &CWeight: &W%d &w(max %d)\n\r",
	get_curr_wis(ch), ch->perm_wis, buf, ch->carry_weight, can_carry_w(ch));

    pager_printf_color(ch, "&CCHA  : &W%2.2d&C(&w%2.2d&C)     Wimpy: &Y%-5d      \n\r",
	get_curr_cha(ch), ch->perm_cha, ch->wimpy);

    pager_printf_color(ch, "&CLCK  : &W%2.2d&C(&w%2.2d&C)\n\r",
	get_curr_lck(ch), ch->perm_lck );

    pager_printf_color(ch, "&CGlory: &W%d&C/&w%d\n\r",
	ch->pcdata->quest_curr, ch->pcdata->quest_accum );

    pager_printf_color(ch, "                   &CHitpoints: &G%-5d &Cof &g%5d   &CPager: (&W%c&C) &W%3d    &CAutoExit(&W%c&C)\n\r",
	ch->hit, ch->max_hit,
	IS_SET(ch->pcdata->flags, PCFLAG_PAGERON) ? 'X' : ' ',
	ch->pcdata->pagerlen, xIS_SET(ch->act, PLR_AUTOEXIT) ? 'X' : ' ');
 ...
 * Return ascii name of an affect location.
 */
const char *get_subclass( int subclass )
{
   switch ( subclass )
   {
      case SUBCLASS_NONE:
      ...
      case SUBCLASS_LIFE:
         return "Elemental(Life)";
      case SUBCLASS_DARKNESS:
         return "Elemental(Darkness)";
      case SUBCLASS_SOUND:
         return "Illusion(Sound)";
      case SUBCLASS_SIGHT:
         return "Illusion(Sight)";
      case SUBCLASS_SMELL:
         return "Illusion(Smell)";
      case SUBCLASS_TOUCH:
         return "Illusion(Touch)";
      case SUBCLASS_CHARM:
         return "Enchanting(Charm)";
      case SUBCLASS_GLAMOUR:
         return "Enchanting(Glamour)";
      case SUBCLASS_ENHANCEMENT:
         return "Enchanting(Enhancement)";
      case SUBCLASS_CREATION:
         return "Nature(Creation)";
      case SUBCLASS_GROWTH:
         return "Nature(Growth)";
      case SUBCLASS_TIME:
         return "Nature(Time)";
      case SUBCLASS_WEATHER:
         return "Nature(Weather)";
   };

   bug( "get_subclass: unknown subclass %d.", subclass );
   return "(?)";
}
...
void do_affected( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg, "by" ) )
   {
      ...
      if( ch->susceptible > 0 )
      {
         send_to_char_color( "&BWeaknesses:   ", ch );
