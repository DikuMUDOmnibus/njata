...
void load_races(  )
{
   ...
   for( i = 0; i < MAX_RACE; ++i )
   {
      if( race_table[i] == NULL )
      {
         ...
         snprintf( race_table[i]->race_name, 16, "%s", "" );
      }
   }
   fclose( fpList );
   return;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'M':
            ...
            KEY( "Move_Regen", race->move_regen, fread_number( fp ) );
...
void fwrite_skill( FILE * fpout, SKILLTYPE * skill )
{
   ...
   if( skill->req_skill && skill->req_skill != -1 && IS_VALID_SN( skill->req_skill ) )
      fprintf( fpout, "ReqSkill     %d\n", skill->req_skill );
   if( skill->min_mana )
      fprintf( fpout, "Mana         %d\n", skill->min_mana );
   if( skill->beats )
...
SKILLTYPE *fread_skill( FILE * fp )
{
   ...
   skill->slot = 0;
   skill->min_mana = 0;
/* Modified so all skills are Level 0 Skills unless they're on class tables -Danny */
   for( x = 0; x < MAX_CLASS; ++x )
   {
      skill->skill_level[x] = 0;
      skill->skill_adept[x] = 100;
   }
   for( x = 0; x < MAX_RACE; ++x )
   {
      skill->race_level[x] = LEVEL_IMMORTAL;
      skill->race_adept[x] = 100;
   }
   skill->guild = -1;
   skill->target = 0;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'A':
            ...
            KEY( "Attribute", skill->attribute, fread_number( fp ) );
            break;
         ...
         case 'R':
            ...
            KEY( "ReqSkill", skill->req_skill, fread_number( fp ) );
...
/* Next three functions used by login history code -Danny */
