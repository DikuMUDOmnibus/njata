...
/*  Fancy version of Wizhelp, sorts by level -Danny  */
void do_wizhelp( CHAR_DATA* ch, const char* argument )
{
	CMDTYPE *cmd;
	int col, hash;
...
/*
 * Force a mortal player to quit -Danny
 */
void do_fquit( CHAR_DATA* ch, const char* argument)
{
   ...
   if( victim->level > 50 )
   {
      send_to_char( "This can't be used on immortals!\r\n", ch );
      return;
   }
   set_char_color( AT_IMMORT, victim );
...
void do_mstat( CHAR_DATA* ch, const char* argument)
{
   ...
   if( IS_NPC( victim ) )
   {
      ...
      pager_printf_color( ch,
                          "&cNumAttacks : &C%d            &cBase XP worth: &C%d\r\n", victim->numattacks, get_exp_worth(victim) );
   }
   ...
   if( !IS_NPC( victim ) )
      ...
      send_to_pager( "\r\n", ch );
   pager_printf_color( ch, "&cSave versus: Fort: &w%d &cRef: &w%d &cWill: &w%d       &cItems: &w(%d/%d)  &cWeight &w(%d/%d)\r\n",
                       victim->saving_fortitude,
                       victim->saving_reflex,
                       victim->saving_will,
...
void do_newbieset( CHAR_DATA* ch, const char* argument)
{
   ...
   if( ( victim->Class == CLASS_SCHOLAR ) || ( victim->Class == CLASS_ROGUE )
       || ( victim->Class == CLASS_ELEMENTALIST ) || ( victim->Class == CLASS_ILLUSIONIST )
       || ( victim->Class == CLASS_ENCHANTER ) )
   {
      obj = create_object( get_obj_index( OBJ_VNUM_SCHOOL_DAGGER ), 1 );
      obj_to_char( obj, victim );
   }
   else if( ( victim->Class == CLASS_CLERIC ) || ( victim->Class == CLASS_DRUID ) || ( victim->Class == CLASS_MONK ) )
   {
      ...
      obj_to_char( obj, victim );
       ...
       || ( victim->Class == CLASS_BARD ) || ( victim->Class == CLASS_CHEVALIER ) || ( victim->Class == CLASS_LANCER ) )
   {
...
void do_showclass( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      {
         ++ct;
         pager_printf( ch, "%2d> %-11s", i, class_table[i]->who_name );
         if( ct % 5 == 0 )
            send_to_pager( "\r\n", ch );
      }
...
*/
   ...
   ch_printf( ch, "CLASS: %s\r\n\r\nPrime Attribute: %-14s  Weapon: %-5d      Guild: %-5d\r\n",
                       Class->who_name, affect_loc_name( Class->attr_prime ), Class->weapon, Class->guild );
   ch_printf( ch, "Second Attribute:  %-14s  Deficient Attribute:  %-14s\r\n",
                       affect_loc_name( Class->attr_second ), affect_loc_name( Class->attr_deficient ) );
   ch_printf( ch, "                                 Thac0 : %-5d     Thac32: %d\r\n",
                       Class->thac0_00, Class->thac0_32 );
   ch_printf( ch, "Hp Min/Hp Max  : %-2d/%-2d           Mana  : %-3s      ExpBase: %d\r\n",
...
void do_setclass( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      send_to_char( "  second, deficient affected resist suscept skill races\r\n", ch );
      return;
   }
   if( is_number( arg1 ) && ( cl = atoi( arg1 ) ) >= 0 && cl < MAX_CLASS )
      Class = class_table[cl];
...
void do_setrace( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg1[0] == '\0' || arg2[0] == '\0' )
   {
      ...
      send_to_char( "  attack     defense    alignment  acplus   race_recall  \r\n", ch );
      send_to_char( "  minalign   maxalign   height     weight   expmultiplier\r\n", ch );
      send_to_char( "  hungermod  thirstmod  fortitude  reflex   will         \r\n", ch );
      return;
   }
...
/* Classes have race restrictions instead of vice versa -Danny
 ...
 */
   ...
   {
      race->saving_fortitude = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "reflex" ) )
   {
      race->saving_reflex = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   ...
   {
      race->saving_will = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
...
void do_showrace( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf( ch, "Saves: (Fortitude) %d (Reflex) %d (Will) %d\r\n",
              race->saving_fortitude, race->saving_reflex, race->saving_will );
