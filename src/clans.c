...
void do_shove( CHAR_DATA* ch, const char* argument)
{
   ...
   if( IS_NPC( ch ) || !IS_SET( ch->pcdata->flags, PCFLAG_PVP ) )
   {
      send_to_char( "Only PVP characters can shove.\r\n", ch );
      return;
   }
   ...
   if( IS_NPC( victim ) || !IS_SET( victim->pcdata->flags, PCFLAG_PVP ) )
   {
      send_to_char( "You can only shove PVP characters.\r\n", ch );
      return;
   }
...
/* Original racial modifications to 'shove' -Danny
...
void do_drag( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !xIS_SET( victim->act, PLR_SHOVEDRAG ) && !IS_SET( victim->pcdata->flags, PCFLAG_PVP ) )
   {
   ...
   if( !IS_SET( ch->pcdata->flags, PCFLAG_PVP ) && IS_SET( victim->pcdata->flags, PCFLAG_PVP ) )
   {
      send_to_char( "You cannot drag a PVP character.\r\n", ch );
      return;
   }

   if( !IS_SET( victim->pcdata->flags, PCFLAG_PVP ) && victim->position > 3 )
   {
   ...
   if( ch->level - victim->level > 5 || victim->level - ch->level > 5 )
   {
      if( IS_SET( victim->pcdata->flags, PCFLAG_PVP ) && IS_SET( ch->pcdata->flags, PCFLAG_PVP ) )
      {
         send_to_char( "There is too great an experience difference for you to even bother.\r\n", ch );
         return;
      }
