...
void advance_level( CHAR_DATA * ch )
{
   ...
   int add_hp, add_mana, add_move, sn;

/* HP gain per level */
   if( ch->Class == CLASS_PALADIN || ch->Class == CLASS_BARD || ch->Class == CLASS_ILLUSIONIST
         || ch->subclass == SUBCLASS_WATER )
      add_hp = con_app[get_curr_cha( ch )].hitp + number_range( 10, 14 );
   else if( ch->Class == CLASS_LANCER || ch->subclass == SUBCLASS_FIRE )
      add_hp = con_app[get_curr_con( ch )].hitp + number_range( 10, 14 );
   else if( ch->Class == CLASS_WARRIOR || ch->subclass == SUBCLASS_EARTH )
      add_hp = con_app[get_curr_str( ch )].hitp + number_range( 10, 14 );
   else if( ch->Class == CLASS_SCHOLAR || ch->Class == CLASS_ENCHANTER )
      add_hp = con_app[get_curr_int( ch )].hitp + number_range( 10, 14 );
   else if( ch->Class == CLASS_CHEVALIER || ch->Class == CLASS_MONK || ch->Class == CLASS_RANGER
	     || ch->Class == CLASS_ROGUE || ch->subclass == SUBCLASS_AIR )
	  add_hp = con_app[get_curr_dex( ch )].hitp + number_range( 10, 14 );
   else if( ch->Class == CLASS_DRUID || ch->Class == CLASS_CLERIC )
      add_hp = con_app[get_curr_wis( ch )].hitp + number_range( 10, 14 );
   else if( ch->Class == CLASS_PALADIN || ch->Class == CLASS_BARD || ch->Class == CLASS_ILLUSIONIST
         || ch->subclass == SUBCLASS_WATER )
      add_hp = con_app[get_curr_cha( ch )].hitp + number_range( 10, 14 );
   else
      add_hp = con_app[get_curr_lck( ch )].hitp + number_range( 10, 14 );

/* Mana gain per level */
   if( ch->Class == CLASS_CHEVALIER || ch->Class == CLASS_LANCER || ch->Class == CLASS_PALADIN )
      add_mana = number_range( 5, ( 2 * get_curr_str( ch ) ) );
   else if( ch->Class == CLASS_MONK || ch->Class == CLASS_WARRIOR )
      add_mana = number_range( 5, ( 2 * get_curr_con( ch ) ) );
   else if( ch->Class == CLASS_BARD )
      add_mana = number_range( 5, ( 2 * get_curr_int( ch ) ) );
   else if( ch->Class == CLASS_ELEMENTALIST || ch->Class == CLASS_ENCHANTER || ch->Class == CLASS_ILLUSIONIST
    || ch->Class == CLASS_RANGER )
      add_mana = number_range( 5, ( 2 * get_curr_wis( ch ) ) );
   else
      add_mana = number_range( 5, ( 2 * get_curr_lck( ch ) ) );

/* End gain per level */
   add_move = number_range( 5, ( 2 * get_curr_lck( ch ) ) );
...
/* Become a dilettante in all learnable skills/spells -Danny*/
   for( sn = 0; sn < num_skills; ++sn )
   {
      if( ch->level >= skill_table[sn]->skill_level[ch->Class] || ch->level >= skill_table[sn]->race_level[ch->race] )
      {
         /* If skill is a language, secret skill, or level 0 skill, then skip */
         if( ( skill_table[sn]->type == SKILL_TONGUE ) || ( SPELL_FLAG( skill_table[sn], SF_SECRETSKILL ) )
   ...
   if( ch->level < LEVEL_IMMORTAL )
   {
      if( IS_VAMPIRE( ch ) )
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your gain is: %d/%d hp, %d/%d bp, %d/%d end.\r\n",
                   add_hp, ch->max_hit, 1, ch->level + 10, add_move, ch->max_move );
      else
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your gain is: %d/%d hp, %d/%d mana, %d/%d end.\r\n",
...
void gain_exp( CHAR_DATA * ch, int gain )
{
   ...
   /*
    * exp cap to prevent any one event from giving enough exp to
...
int hit_gain( CHAR_DATA * ch )
{
   ...
   else
   {
      ...
      statmod = get_curr_con( ch ) - 10;
      if( statmod > 0 )
         gain += ( statmod * ch->max_hit );
...
/*  Might use later to give Dryads increased regen in sunlight -Danny
      if( IS_VAMPIRE( ch ) )
      {
...
*/
	  if( ch->race == RACE_DWARF || ch->race == RACE_HALF_ELF || ch->race == RACE_DOLLFOLK || ch->race == RACE_FAIRY )
	     gain -= ( 5 * ch->max_hit );

	  if( ch->race == RACE_BENN || ch->race == RACE_SLIME )
         gain += ( 5 * ch->max_hit );

      if( ch->subrace == SUBRACE_SUCCULENT_DRYAD )
         gain += ( 5 * ch->max_hit );
...
int mana_gain( CHAR_DATA * ch )
{
   ...
   else
   {
      ...
      statmod = get_curr_wis( ch ) - 10;
      if( statmod > 0 )
         gain += ( statmod * ch->max_mana );
	  ...
	  if( ch->race == RACE_GNOME || ch->race == RACE_CYCLOPS || ch->race == RACE_DOLLFOLK || ch->race == RACE_SLIME )
	     gain -= ( 5 * ch->max_mana );

	  if( ch->race == RACE_FIREBIRD || ch->race == RACE_FAIRY || ch->race == RACE_SPHINX || ch->race == RACE_UNICORN )
         gain += ( 5 * ch->max_mana );
...
int move_gain( CHAR_DATA * ch )
{
   ...
   else
   {
      ...
      statmod = get_curr_lck( ch ) - 10;
      if( statmod > 0 )
         gain += ( statmod * ch->max_move );
...
*/
	  if( ch->race == RACE_HUMAN || ch->race == RACE_DWARF || ch->race == RACE_BENN || ch->race == RACE_SLIME )
	     gain -= ( 5 * ch->max_move );

	  if( ch->race == RACE_ELF || ch->race == RACE_HALF_ELF || ch->race == RACE_WOLFFOLK || ch->race == RACE_DRYAD
	   || ch->race == RACE_DOLLFOLK || ch->race == RACE_MINOTAUR || ch->race == RACE_CENTAUR || ch->race == RACE_UNICORN )
         gain += ( 5 * ch->max_move );
...
void char_update( void )
{
   ...
   for( ch = last_char; ch; ch = trvch_wnext( lc ) )
   {
      ...
      /* Keep alive function, hopefully prevent disconnects -Danny */
      if (!IS_NPC(ch) && ch->desc != NULL)
         write_to_descriptor( ch->desc, "", 1 );
      ...
      if( !IS_NPC( ch ) && ch->level < LEVEL_IMMORTAL )
      {
         ...
         if( ( obj = get_eq_char( ch, WEAR_HOLD ) ) != NULL && obj->item_type == ITEM_LIGHT && obj->value[2] > 0 )
         {
      ...
      else
      {
         if( ch->mount && ch->in_room != ch->mount->in_room )
         {
			if ( IS_NPC( ch->mount ) )
               ...
               xREMOVE_BIT( ch->mount->act, PLR_MOUNTED );
            ch->mount = NULL;
            ch->position = POS_STANDING;
         ...
         if( ch->in_room
             && ( ( ch->in_room->sector_type == SECT_WATER_NOSWIM ) || ( ch->in_room->sector_type == SECT_WATER_SWIM ) || ( ch->in_room->sector_type == SECT_OCEAN ) ) )
         {
...
//                        mov = UMAX( 1, mov );
                        mov = UMAX( 1, ch->max_move / 20 );
                        if( ch->move - mov < 0 )
                        {
                           send_to_char( "You run out of endurance.\r\n", ch );
                           ch->move = 0;
					    }
                        else
                        {
                           send_to_char( "You tread water.\r\n", ch );
                           ch->move -= mov;
				        }
                     }
                     else
      ...
      for( d = first_descriptor; d; d = d->next )
      {
         if( d->connected == CON_PLAYING && IS_OUTSIDE( d->character ) && IS_AWAKE( d->character ) )
         {
            ...
            switch( time_info.hour )
            {
               ...
               {
                  const char *echo_strings[4] = {
                     "The fiery disc slowly disappears in the west.\r\n",
                     "The reddish fiery disc sets past the horizon.\r\n",
                     "The sky turns a reddish orange as the fiery disc ends its journey.\r\n",
                     "The fiery disc's radiance dims as it sinks in the sky.\r\n"
                  };
               ...
               case 19:
               {
                  ...
                  {
                     const char *echo_strings[2] = {
                        "The gentle glow of Nulle diffuses through the night sky.\r\n",
