...
/* Maybe we should delete the next line */
   rval = p;
   /*
    * while ( *p && !isspace(*p) ) ++p;
    */
...
int mprog_command_check(const char *arg, CHAR_DATA*mob, CHAR_DATA *actor,
...
signed int prog_command_trigger( CHAR_DATA *ch, const char *txt, bool bExact )
{
    CHAR_DATA* vch;
    int count = 0;
	...
	if ( ch->in_room == NULL )
	{
        bug( "ERROR! prog_command_trigger ch %s has null in_room!", ch->name );
		return -1;
	}

    if( HAS_PROG( ch->in_room, COMMAND_PROG ) )
    {
/*        rset_supermob(ch->GetInRoom());*/
        count += mprog_command_check(txt, supermob, ch, NULL, ch->in_room->mudprogs, bExact);
/*        release_supermob();*/
    }

	if ( ch->in_room == NULL )
	{
        bug( "ERROR! prog_command_trigger ch %s has null in_room after command trigger!", ch->name );
		return -1;
	}
    ...
    for ( vch = ch->in_room->first_person; vch; vch = vch->next_in_room )
    {
        ...
        if ( !( HAS_PROG( vch->in_room, COMMAND_PROG ) ) )
            continue;
...
/* Used to allow the standard command to trigger after the command prog -Danny */
