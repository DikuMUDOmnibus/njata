   ...
   else
   {
      ...
      switch ( container->item_type )
      {
         ...
         case ITEM_CORPSE_PC:
         {
            ...
            if( IS_OBJ_STAT( container, ITEM_CLANCORPSE )
                && !IS_NPC( ch ) && IS_SET( ch->pcdata->flags, PCFLAG_PVP )
...
/* Checks for two handed weapons */
   if( wield && IS_OBJ_STAT( wielded, ITEM_TWOHANDED ) )
   {
      send_to_char( "You're wielding a two handed weapon already!\r\n", ch );
      return FALSE;
   }
   /*
   ...
   {
      send_to_char( "You cannot hold another weapon, you're already holding something there!\r\n", ch );
      return FALSE;
   }
   return TRUE;
}
...
void wear_obj( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, short wear_bit )
{
   ...
   if( !IS_IMMORTAL( ch )
       && ( ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && ch->Class == CLASS_WARRIOR )
            ...
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && ch->Class == CLASS_CHEVALIER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && ch->Class == CLASS_LANCER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && ch->Class == CLASS_RANGER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && ch->Class == CLASS_SCHOLAR )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && ch->Class == CLASS_ELEMENTALIST )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && ch->Class == CLASS_ENCHANTER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && ch->Class == CLASS_ILLUSIONIST )
   ...
   if( IS_OBJ_STAT( obj, ITEM_TINY ) && ch->height > 12 )
   {
      act( AT_MAGIC, "You are much too large to use that item.", ch, NULL, NULL, TO_CHAR );
      return;
   }
   ...
   switch ( 1 << bit )
   {
      ...
      case ITEM_WEAR_FINGER:
         if( get_eq_char( ch, WEAR_FINGER_L )
            ...
            return;
/* Non-Centaur Bipedal races have no ring fingers! -Danny */
   		 if( ch->race == RACE_SPHINX || ch->race == RACE_GRYPHON || ch->race == RACE_DRAGON || ch->race == RACE_UNICORN )
   		 {
      		send_to_char( "You don't have fingers!\r\n", ch );
      		return;
   		 }
         if( !get_eq_char( ch, WEAR_FINGER_L ) )
         {
      ...
      case ITEM_WEAR_NECK:
         ...
         send_to_char( "You're already wearing a neck item.\r\n", ch );
         return;
    ...
    /* Naga don't have legs -Danny */
		 if( ch->race == RACE_NAGA )
   		 {
      		send_to_char( "You have no legs!\r\n", ch );
      		return;
   		 }
    /* Merfolk only have legs some of the time! -Danny */
		 if( ch->race == RACE_MERFOLK && !xIS_SET( ch->act, PLR_HASLEGS ) )
   		 {
      		send_to_char( "You have no legs at the moment!\r\n", ch );
      		return;
   		 }
    /* Bipedal races need special leg clothes -Danny */
   		 if( ch->race == RACE_CENTAUR || ch->race == RACE_SPHINX || ch->race == RACE_GRYPHON
   		 						      || ch->race == RACE_DRAGON || ch->race == RACE_UNICORN )
   		 {
      		send_to_char( "These appear to be designed for creatures with only two legs.\r\n", ch );
      		return;
   		 }
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
      ...
      case ITEM_WEAR_TAIL:
         if( !can_layer( ch, obj, WEAR_TAIL ) )
         {
            send_to_char( "It won't fit overtop of what you're already wearing.\r\n", ch );
            return;
         }
    /* Only Naga have tails, and Merfolk(sometimes!) -Danny */
		 if( ch->race != RACE_NAGA && ch->race != RACE_MERFOLK )
   		 {
      		send_to_char( "You don't have a tail!\r\n", ch );
      		return;
   		 }
    /* Merfolk only have tails some of the time! -Danny */
 		 if( ch->race == RACE_MERFOLK && xIS_SET( ch->act, PLR_HASLEGS ) )
   		 {
      		send_to_char( "You don't currently have a tail!\r\n", ch );
      		return;
   		 }
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
	...
	/* Naga don't have feet. Satyrs, Centaurs, Minotaurs, and Unicorns have hooves. -Danny  */
		 if( ch->race == RACE_NAGA || ch->race == RACE_SATYR || ch->race == RACE_CENTAUR
		                           || ch->race == RACE_UNICORN || ch->race == RACE_MINOTAUR )
   		 {
      		send_to_char( "You have no feet!\r\n", ch );
      		return;
   		 }
    /* Merfolk can sometimes have legs -Danny */
		 if( ch->race == RACE_MERFOLK && !xIS_SET( ch->act, PLR_HASLEGS ) )
   		 {
      		send_to_char( "You have no feet at the moment!\r\n", ch );
      		return;
   		 }
    /* Firebird race has taloned bird feet, can't wear shoes! */
 		 if( ch->race == RACE_FIREBIRD )
   		 {
      		send_to_char( "Your talons wouldn't fit in those.\r\n", ch );
      		return;
   		 }
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
      ...
      case ITEM_WEAR_HOOVES:
         if( !can_layer( ch, obj, WEAR_HOOVES ) )
         {
            send_to_char( "It won't fit overtop of what you're already wearing.\r\n", ch );
            return;
         }
	/* Only Satyrs, Centaurs, Minotaurs, and Unicorns have hooves. -Danny  */
		 if( ch->race != RACE_SATYR && ch->race != RACE_CENTAUR && ch->race != RACE_UNICORN && ch->race != RACE_MINOTAUR )
   		 {
      		send_to_char( "You have no hooves!\r\n", ch );
      		return;
   		 }
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
            act( AT_ACTION, "$n wears $p on $s hooves.", ch, obj, NULL, TO_ROOM );
            act( AT_ACTION, "You wear $p on your hooves.", ch, obj, NULL, TO_CHAR );
         }
...
/* Bipedal races have no hands either! -Danny */
   		 if( ch->race == RACE_SPHINX || ch->race == RACE_GRYPHON || ch->race == RACE_DRAGON || ch->race == RACE_UNICORN )
   		 {
      		send_to_char( "You have no hands!\r\n", ch );
      		return;
   		 }
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
...
/* Non-centaur bipedal races have no arms -Danny */
   		 if( ch->race == RACE_SPHINX || ch->race == RACE_GRYPHON || ch->race == RACE_DRAGON || ch->race == RACE_UNICORN )
   		 {
      ...
      case ITEM_WEAR_WRIST:
         if( get_eq_char( ch, WEAR_WRIST_L )
            ...
            return;
/* Non-Centaur bipedal races have no wrists -Danny */
   		 if( ch->race == RACE_SPHINX || ch->race == RACE_GRYPHON || ch->race == RACE_DRAGON || ch->race == RACE_UNICORN )
   		 {
      		send_to_char( "You don't have wrists!\r\n", ch );
      		return;
   		 }
         if( !get_eq_char( ch, WEAR_WRIST_L ) )
         {
      ...
      case ITEM_WEAR_ANKLES:
         if( get_eq_char( ch, WEAR_FORE_ANKLES )
             && get_eq_char( ch, WEAR_HIND_ANKLES )
             && !remove_obj( ch, WEAR_FORE_ANKLES, fReplace ) && !remove_obj( ch, WEAR_HIND_ANKLES, fReplace ) )
            return;
/* Let's give bipedal races their own wear locations */
   		 if( ch->race != RACE_SPHINX && ch->race != RACE_GRYPHON && ch->race != RACE_DRAGON )
   		 {
      		send_to_char( "These appear to be designed for bipedal creatures without hooves.\r\n", ch );
      		return;
   		 }
         if( !get_eq_char( ch, WEAR_FORE_ANKLES ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n fits $p around $s fore ankles.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You fit $p around your fore ankles.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_FORE_ANKLES );
            oprog_wear_trigger( ch, obj );
            return;
         }

         if( !get_eq_char( ch, WEAR_HIND_ANKLES ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n fits $p around $s hind ankles.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You fit $p around your hind ankles.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_HIND_ANKLES );
            oprog_wear_trigger( ch, obj );
            return;
         }

         bug( "%s: no free ankle.", __FUNCTION__ );
         send_to_char( "You already wear two pairs of ankle items.\r\n", ch );
         return;

      case ITEM_WEAR_LIMBS:
         if( get_eq_char( ch, WEAR_FORE_LEGS )
             && get_eq_char( ch, WEAR_HIND_LEGS )
             && !remove_obj( ch, WEAR_FORE_LEGS, fReplace ) && !remove_obj( ch, WEAR_HIND_LEGS, fReplace ) )
            return;
/* Bipedal races and centaurs have two sets of legs -Danny */
   		 if( ch->race != RACE_CENTAUR && ch->race != RACE_SPHINX && ch->race != RACE_GRYPHON
   		                              && ch->race != RACE_DRAGON && ch->race != RACE_UNICORN )
   		 {
      		send_to_char( "These appear to be designed for bipedal limbs.\r\n", ch );
      		return;
   		 }
         if( !get_eq_char( ch, WEAR_FORE_LEGS ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n fits $p onto $s fore legs.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You fit $p onto your fore legs.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_FORE_LEGS );
            oprog_wear_trigger( ch, obj );
            return;
         }

         if( !get_eq_char( ch, WEAR_HIND_LEGS ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n fits $p onto $s hind legs.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You fit $p onto your hind legs.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_HIND_LEGS );
            oprog_wear_trigger( ch, obj );
            return;
         }

         bug( "%s: no free ankle.", __FUNCTION__ );
         send_to_char( "You already wear two pairs of limb items.\r\n", ch );
         return;

      case ITEM_WEAR_SHIELD:
         if( get_eq_char( ch, WEAR_WIELD ) && IS_OBJ_STAT( get_eq_char( ch, WEAR_WIELD ), ITEM_TWOHANDED ) )
         {
            send_to_char( "You can't use a shield while wielding a two-handed weapon.\r\n", ch );
            return;
         }
      ...
      case ITEM_WIELD:
         ...
         else
         {
            ...
            if( hd && IS_OBJ_STAT( obj, ITEM_TWOHANDED ) )
            {
               send_to_char( "You can't wield a two-handed weapon while holding something.\r\n", ch );
               return;
            }

            if( sd && IS_OBJ_STAT( obj, ITEM_TWOHANDED ) )
            {
               send_to_char( "You can't wield a two-handed weapon with a shield.\r\n", ch );
               return;
            }
            ...
            if( tmpobj )
            {
               ...
               {
                  send_to_char( "That weapon requires both hands to wield.\r\n", ch );
                  return;
               }
               ...
               {
                  send_to_char( "You're already wielding a two-handed weapon.\r\n", ch );
                  return;
               }
      ...
      case ITEM_HOLD:
         if( get_eq_char( ch, WEAR_WIELD ) && IS_OBJ_STAT( get_eq_char( ch, WEAR_WIELD ), ITEM_TWOHANDED ) )
         {
            send_to_char( "You can't hold something while wielding a two-handed weapon.\r\n", ch );
            return;
         }
         if( get_eq_char( ch, WEAR_DUAL_WIELD )
         ...
         if( obj->item_type == ITEM_WAND
             ...
             || obj->item_type == ITEM_MEAT
             || obj->item_type == ITEM_FISH
         ...
         {
            act( AT_ACTION, "$n holds $p.", ch, obj, NULL, TO_ROOM );
            act( AT_ACTION, "You hold $p.", ch, obj, NULL, TO_CHAR );
         }
...
void do_bury( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      if( !IS_OBJ_STAT( obj, ITEM_CLANCORPSE ) || IS_NPC( ch ) || !IS_SET( ch->pcdata->flags, PCFLAG_PVP ) )
      {
