...
void fwrite_morph( FILE * fp, MORPH_DATA * morph )
{
   ...
   if( morph->parry != 0 )
      fprintf( fp, "Parry        	%d\n", morph->parry );
   if( morph->saving_fortitude != 0 )
      fprintf( fp, "SaveFort            %d\n", morph->saving_fortitude );
   if( morph->saving_reflex != 0 )
      fprintf( fp, "SaveReflex        	%d\n", morph->saving_reflex );
   if( morph->saving_will != 0 )
      fprintf( fp, "SaveWill            %d\n", morph->saving_will );
   if( morph->str != 0 )
...
void do_morphset( CHAR_DATA* ch, const char* argument)
{
   ...
   else if( !str_cmp( arg2, "fort" ) || !str_cmp( arg2, "fortsave" ) )
   {
      ...
      morph->saving_fortitude = value;
   }
   else if( !str_cmp( arg2, "reflex" ) || !str_cmp( arg2, "reflexsave" ) )
   {
      if( value < -30 || value > 30 )
      {
      ...
      morph->saving_reflex = value;
   }
   else if( !str_cmp( arg2, "will" ) || !str_cmp( arg2, "willsave" ) )
   {
      if( value < -30 || value > 30 )
      {
...
void do_morphstat( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      pager_printf( ch, "  &cSave versus: Fort: &w%d &cRef: &w%d &cWill: &w%d       &cDodge: &w%d  &cParry: &w%d  &cTumble: &w%d\r\n",
                    morph->saving_fortitude, morph->saving_reflex, morph->saving_will,
...
CHAR_MORPH *make_char_morph( MORPH_DATA * morph )
{
   ...
   ch_morph->saving_fortitude = morph->saving_fortitude;
   ch_morph->saving_reflex = morph->saving_reflex;
   ch_morph->saving_will = morph->saving_will;
...
void do_morph( CHAR_DATA * ch, MORPH_DATA * morph )
{
   ...
   ch->saving_fortitude += morph->saving_fortitude;
   ch->saving_reflex += morph->saving_reflex;
   ch->saving_will += morph->saving_will;
   ch_morph->hitroll = morph->hitroll ? dice_parse( ch, morph->level, morph->hitroll, NULL, 0 ) : 0;
   ch->hitroll += ch_morph->hitroll;
   ch_morph->damroll = morph->damroll ? dice_parse( ch, morph->level, morph->damroll, NULL, 0 ) : 0;
   ch->damroll += ch_morph->damroll;
   ch_morph->hit = morph->hit ? dice_parse( ch, morph->level, morph->hit, NULL, 0 ) : 0;
   if( ( ch->hit + ch_morph->hit ) > 32700 )
      ch_morph->hit = ( 32700 - ch->hit );
   ch->hit += ch_morph->hit;
   ch_morph->move = morph->move ? dice_parse( ch, morph->level, morph->move, NULL, 0 ) : 0;
   if( ( ch->move + ch_morph->move ) > 32700 )
      ch_morph->move = ( 32700 - ch->move );
   ch->move += ch_morph->move;

   ch_morph->mana = morph->mana ? dice_parse( ch, morph->level, morph->mana, NULL, 0 ) : 0;
...
void morph_defaults( MORPH_DATA * morph )
{
   ...
   morph->level = 0;
   morph->parry = 0;
   morph->saving_fortitude = 0;
   morph->saving_reflex = 0;
   morph->saving_will = 0;
   morph->str = 0;
   morph->tumble = 0;
   morph->wis = 0;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'R':
            ...
            break;
         case 'S':
            KEY( "SaveFort", morph->saving_fortitude, fread_number( fp ) );
            KEY( "SaveReflex", morph->saving_reflex, fread_number( fp ) );
            KEY( "SaveWill", morph->saving_will, fread_number( fp ) );
...
void copy_morph( MORPH_DATA * morph, MORPH_DATA * temp )
{
   ...
   morph->saving_fortitude = temp->saving_fortitude;
   morph->saving_reflex = temp->saving_reflex;
   morph->saving_will = temp->saving_will;
   morph->str = temp->str;
