...
void do_eat( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !IS_IMMORTAL( ch ) )
   {
	  ...
	  if( ( obj->item_type == ITEM_MEAT || obj->item_type == ITEM_FISH ) && ( ( ch->race == RACE_FIREBIRD ) || ( ch->race == RACE_FAIRY ) || ( ch->race == RACE_UNICORN ) ) )
	  {
		 send_to_char( "You crinkle up your nose and try your hardest but can't do it.\r\n", ch );
		 return;
	  }

	  if( ( obj->item_type == ITEM_PLANT ) && ( ( ch->race == RACE_CATFOLK ) || ( ch->race == RACE_NAGA ) || ( ch->race == RACE_LIZARDFOLK )
	       || ( ch->race == RACE_SPHINX ) || ( ch->race == RACE_SALAMANDER ) || ( ch->race == RACE_GRYPHON ) || ( ch->race == RACE_DRAGON ) ) )
	  {
		 send_to_char( "You sniff it gingerly, but it just doesn't seem all that appetizing.\r\n", ch );
		 return;
	  }
...
void do_quaff( CHAR_DATA* ch, const char* argument)
{
   ...
   /*
      Removed Fullness Checking		-Danny
