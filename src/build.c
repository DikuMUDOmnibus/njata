...
const char *const ex_flags[] = {
   ...
   "bashed", "bashproof", "nomob", "window", "can_look", "isbolt", "bolted", "tiny"
};

const char *const sec_flags[] = {
   "inside", "city", "field", "forest", "hills", "mountain", "water_swim",
   "water_noswim", "underwater", "air", "desert", "ocean", "oceanfloor",
...
const char *const w_flags[] = {
   ...
   "missile", "back", "face", "ankles", "tail", "hooves", "limbs",
   "r7", "r8", "r9", "r10", "r11", "r12", "r13"
};

const char *const item_w_flags[] = {
   "take", "face", "head", "eyes", "ears", "neck", "about", "body", "back",
   "waist", "arms", "limbs", "hands", "ankles", "wrist", "wrist", "finger",
   "finger", "legs", "limbs", "feet", "tail", "hooves", "ankles", "held", "wield",
   "dual", "shield", "missile", "r7", "r8", "r9", "r10", "r11", "r12", "r13"
};
...
const char *const o_types[] = {
   ...
   "flint", "_empty1", "_empty2", "missileweapon", "projectile", "quiver", "shovel",
   "salve", "cook", "keyring", "odor", "chance", "meat", "plant", "hammer",
   "ingot", "axe", "wood", "fish", "fishingpole" ,"mix"
};
...
const char *const a_flags[] = {
   ...
   "charm", "flying", "pass_door", "floating", "truesight", "darkvision", "detect_traps",
...
const char *const act_flags[] = {
   ...
   "pacifist", "noattack", "annoying", "statshield", "prototype", "train"
};

const char *const pc_flags[] = {
   "r1", "pvp", "unauthed", "norecall", "nointro", "gag", "retired", "guest",
...
const char *const plr_flags[] = {
   ...
   "flee", "autogold", "automap", "afk", "invisprompt", "compass", "mounted",
   "mountable", "haslegs"
};
...
const char *const wear_locs[] = {
   "face", "head", "eyes", "ears", "neck", "about", "body", "back",
   "waist", "arms", "limbs1", "hands", "ankles1", "wrist1", "wrist2",
   "finger1", "finger2", "legs", "limbs2", "feet", "tail", "hooves",
   "ankles2", "hold", "wield", "dual_wield", "shield", "missile_wield"
};

const char *const ris_flags[] = {
   "fire", "water", "air", "earth", "life", "darkness", "blunt", "pierce", "slash",
   "poison", "sleep", "charm", "hold", "nonmagic", "plus1", "plus2",
   "plus3", "plus4", "plus5", "plus6", "magic", "paralysis", "hack", "lash", "r3",
...
void start_editing( CHAR_DATA * ch, const char *data )
{
   ...
   if( !data )
      ...
      for( ;; )
      {
         ...
         if( lines >= 100 || size > 4096 )
         {
            edit->line[lines][lpos] = '\0';
            break;
         }
      }
   if( lpos > 0 && lpos < 78 && lines < 100 )
   {
...
void do_mset( CHAR_DATA* ch, const char* argument)
{
   ...
   switch ( ch->substate )
   {
      ...
      case SUB_MOB_DESC:
         if( !ch->dest_buf )
         {
            send_to_char( "Fatal error: report to njatazoie@gmail.com\r\n", ch );
   ...
   if( arg1[0] == '\0' || ( arg2[0] == '\0' && ch->substate != SUB_REPEATCMD ) || !str_cmp( arg1, "?" ) )
   {
      ...
      send_to_char( "  str int wis dex con cha lck sex class subclass\r\n", ch );
      ...
      send_to_char( "  fortitude reflex will (see SAVINGTHROWS)\r\n", ch );
      send_to_char( "  resistant immune susceptible (see RIS)\r\n", ch );
      send_to_char( "  attack defense numattacks height weight bodyshape age\r\n", ch );
   ...
   if( !str_cmp( arg2, "fort" ) || !str_cmp( arg2, "fortitude" ) )
   {
      if( !can_mmodify( ch, victim ) )
         return;
      if( value < -30 || value > 30 )
      {
         send_to_char( "Fortitude saving throw range is -30 to 30.\r\n", ch );
         return;
      }
      victim->saving_fortitude = value;
      if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
         victim->pIndexData->saving_fortitude = value;
      return;
   }

   if( !str_cmp( arg2, "ref" ) || !str_cmp( arg2, "reflex" ) )
   {
      if( !can_mmodify( ch, victim ) )
         return;
      if( value < -30 || value > 30 )
      {
         send_to_char( "Reflex saving throw range is -30 to 30.\r\n", ch );
         return;
      }
      victim->saving_reflex = value;
      if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
         victim->pIndexData->saving_reflex = value;
      return;
   }
   ...
   if( !str_cmp( arg2, "will" ) )
   {
      ...
      if( value < -30 || value > 30 )
      {
         send_to_char( "Will saving throw range is -30 to 30.\r\n", ch );
         return;
      }
      victim->saving_will = value;
      if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
         victim->pIndexData->saving_will = value;
      return;
   }
...
/* Begin Njata specific stuff -Danny */

   if( !str_cmp( arg2, "subclass" ) )
   {
      if( !can_mmodify( ch, victim ) )
         return;
      ...
      if( IS_NPC( victim ) )
      {
         send_to_char( "NPCs don't currently use subclasses.\r\n", ch );
         return;
      }

      if( value < 0 || value >= SUBCLASS_MAX )
      {
         ch_printf( ch, "Subclass range is 0 to %d.\n", SUBCLASS_MAX - 1 );
         return;
      }

      victim->subclass = value;
      return;
   }
   ...
   if( !str_cmp( arg2, "height" ) )
   {
      ...
      {
         send_to_char( "Height must be between 1 and 120 inches.\r\n", ch );
         return;
      }
      victim->height = value;
      if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
   ...
   if( !str_cmp( arg2, "weight" ) )
   {
      if( !can_mmodify( ch, victim ) )
         return;
      if( value < 1 || value > 2200 )
      {
         send_to_char( "Weight must be between 1 and 2200 pounds.\r\n", ch );
         return;
      }
      victim->weight = value;
      if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
   ...
   if( !str_cmp( arg2, "bodyshape" ) )
   {
      ...
      if( value < 0 || value > 14 )
      {
         ch_printf( ch, "Body shape range is 0 to 14.\n" );
         return;
      }
      victim->bodyshape = value;
   ...
   if( !str_cmp( arg2, "age" ) )
   {
      ...
      if( value < 0 || value > 4 )
      {
         ch_printf( ch, "Age category range is 0 to 4.\n" );
         return;
      }
      victim->pcdata->age_category = value;

      return;
   }

/* End Njata specific stuff -Danny */
   ...
   if( !str_cmp( arg2, "pkill" ) )
   {
      ...
      {
         REMOVE_BIT( victim->pcdata->flags, PCFLAG_PVP );
         xSET_BIT( victim->act, PLR_NICE );
         send_to_char( "You are now a non-PVP player.\r\n", victim );
         if( ch != victim )
            send_to_char( "That player is now non-PVP.\r\n", ch );
      }
      else
      {
         SET_BIT( victim->pcdata->flags, PCFLAG_PVP );
         xREMOVE_BIT( victim->act, PLR_NICE );
         send_to_char( "You are now a PVP player.\r\n", victim );
         if( ch != victim )
            send_to_char( "That player is now PVP.\r\n", ch );
      }
      ...
      while( argument[0] != '\0' )
      {
         ...
         else
         {
            if( value == ITEM_PROTOTYPE
                && get_trust( ch ) < sysdata.level_modify_proto && !is_name( "protoflag", ch->pcdata->bestowments ) )
               send_to_char( "You cannot change the prototype flag.\r\n", ch );
            else
            {
...
void do_redit( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg, "flags" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         {
            if( value == ROOM_PROTOTYPE && get_trust( ch ) < sysdata.level_modify_proto )
               send_to_char( "You cannot change the prototype flag.\r\n", ch );
            else
    ...
    else
    {
		if ( color_strlen(argument) > 75 )
		{
          	...
          	while ( argument[0] != '\0' )
	  		{
	     		...
	     		if (color_strlen(arg) > 75) // A single word more than 80 long? Skip!
               	continue;

	     		if ((color_strlen(buf) + color_strlen(arg) + 1) <= 75 )
...
void fwrite_fuss_mobile( FILE * fpout, MOB_INDEX_DATA * pMobIndex, bool install )
{
   ...
   fprintf( fpout, "Saves      %d %d %d\n",
            pMobIndex->saving_fortitude, pMobIndex->saving_reflex, pMobIndex->saving_will );
   if( pMobIndex->speaks )
...
void old_fold_area( AREA_DATA * tarea, char *filename, bool install )
{
   ...
   for( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
   {
      ...
      if( pMobIndex->perm_str != 10 || pMobIndex->perm_int != 10
          || pMobIndex->perm_wis != 10 || pMobIndex->perm_dex != 10
          || pMobIndex->perm_con != 10 || pMobIndex->perm_cha != 10
      ...
      if( complexmob )
      {
         fprintf( fpout, "%d %d %d %d %d %d %d\n",
                  ...
                  pMobIndex->perm_wis, pMobIndex->perm_dex, pMobIndex->perm_con, pMobIndex->perm_cha, pMobIndex->perm_lck );
         fprintf( fpout, "%d %d %d\n",
                  pMobIndex->saving_fortitude,
                  pMobIndex->saving_reflex,
                  pMobIndex->saving_will );
         fprintf( fpout, "%d %d %d %d %d %d %d\n",
                  pMobIndex->race,
                  pMobIndex->Class,
   ...
   for( vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++ )
   {
      ...
      fprintf( fpout, "%d %s %d %d %d\n", pObjIndex->item_type,
               print_bitvector( &pObjIndex->extra_flags ), pObjIndex->wear_flags, pObjIndex->layers, pObjIndex->level );
