...
void do_mpmset( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg2, "fort" ) || !str_cmp( arg2, "fortitude" ) )
   {
      ...
      {
         progbug( "MpMset: Invalid fort save", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "ref" ) || !str_cmp( arg2, "reflex" ) )
   {
      if( value < -30 || value > 30 )
      {
         progbug( "MpMset: Invalid reflex save", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "will" ) )
   {
      if( value < -30 || value > 30 )
      {
         progbug( "MpMset: Invalid will save", ch );
         return;
      }
...
/* Begin Njata specific stuff -Danny */
   ...
   if( !str_cmp( arg2, "subclass" ) )
   {
      if( IS_NPC( victim ) )
      {
         progbug( "MpMset: can't set npc subclass", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "height" ) )
   {
      if( IS_NPC( victim ) )
      {
         progbug( "MpMset: can't set npc height", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "weight" ) )
   {
      if( IS_NPC( victim ) )
      {
         progbug( "MpMset: can't set npc weight", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "bodyshape" ) )
   {
      if( IS_NPC( victim ) )
      {
         progbug( "MpMset: can't set npc bodyshape", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "age" ) )
   {
      if( IS_NPC( victim ) )
      {
         progbug( "MpMset: can't set npc age", ch );
         return;
      }
...
/*
 * New mpsset command for allowing mobs to "teach" skills! -Danny
 */
void do_mpsset( CHAR_DATA* ch, const char* argument)
{
   char arg1[MAX_INPUT_LENGTH];
   char arg2[MAX_INPUT_LENGTH];
   char arg3[MAX_INPUT_LENGTH];
   ...
   if( !*arg1 )
   {
      progbug( "MpSset: no args", ch );
      return;
   }

   if( ( victim = get_char_room( ch, arg1 ) ) == NULL )
   {
      progbug( "MpSset: no victim", ch );
      return;
   }

   if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
   {
      progbug( "MpSset: victim is proto", ch );
      return;
   }

   if( IS_NPC( victim ) )
   {
      progbug( "MpSset: victim is an npc", ch );
      return;
   }
