...
short gsn_russa;
short gsn_wolof;
short gsn_corustenya;
short gsn_simgh;
short gsn_askravaardi;
short gsn_grega;
short gsn_wildtongue;
short gsn_yranib;
short gsn_mij;
short gsn_sauloon;
short gsn_vedric;
...
void boot_db( bool fCopyOver )
{
    ...
    */
   {
      ...
      ASSIGN_GSN( gsn_colistani, "colistani" );
      ASSIGN_GSN( gsn_caolidan, "caolidan" );
      ASSIGN_GSN( gsn_dorish, "dorish" );
      ASSIGN_GSN( gsn_dardi, "dardi" );
      ASSIGN_GSN( gsn_lavandai, "lavandai" );
      ASSIGN_GSN( gsn_valao, "valao" );
      ASSIGN_GSN( gsn_ftaghn, "ftaghn" );
      ASSIGN_GSN( gsn_fahn, "fahn" );
      ASSIGN_GSN( gsn_vaolumin, "vaolumin" );
      ASSIGN_GSN( gsn_chya, "chya" );
      ASSIGN_GSN( gsn_fenradya, "fenradya" );
      ASSIGN_GSN( gsn_siru, "siru" );
      ASSIGN_GSN( gsn_solese, "solese" );
      ASSIGN_GSN( gsn_russa, "russa" );
      ASSIGN_GSN( gsn_wolof, "wolof" );
      ASSIGN_GSN( gsn_corustenya, "corustenya" );
      ASSIGN_GSN( gsn_simgh, "simgh" );
      ASSIGN_GSN( gsn_askravaardi, "askravaardi" );
      ASSIGN_GSN( gsn_grega, "grega" );
      ASSIGN_GSN( gsn_wildtongue, "wildtongue" );
      ASSIGN_GSN( gsn_yranib, "yranib" );
      ASSIGN_GSN( gsn_mij, "mij" );
      ASSIGN_GSN( gsn_sauloon, "sauloon" );
      ASSIGN_GSN( gsn_vedric, "vedric" );
   }
...
void load_mobiles( AREA_DATA * tarea, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      if( letter == 'C' )  /* Realms complex mob   -Thoric */
      {
         ...
         pMobIndex->saving_fortitude = fread_number( fp );
         pMobIndex->saving_reflex = fread_number( fp );
         pMobIndex->saving_will = fread_number( fp );
         ...
         else  /* SmaugWiz zone here */
         {
            ...
            if( !str_cmp( ln, "all" ) )
               pMobIndex->speaks = ~LANG_COLISTANI;
            else
            {
               while( ln[0] != '\0' )
               {
            ...
            if( !str_cmp( ln, "all" ) )
               pMobIndex->speaking = ~LANG_COLISTANI;
            else
            {
   ...
   for( pArea = first_area; pArea; pArea = pArea->next )
   {
      ...
      if( pArea->nplayer > 0 && pArea->age == ( reset_age - 1 ) )
      {
         ...
         for( pch = first_char; pch; pch = pch->next )
         {
            ...
            {
               if ( xIS_SET(pch->act, PLR_RESETMSG) )
               {
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA * pMobIndex )
{
   ...
   mob->saving_fortitude = pMobIndex->saving_fortitude;
   mob->saving_reflex = pMobIndex->saving_reflex;
   mob->saving_will = pMobIndex->saving_will;
...
void clear_char( CHAR_DATA * ch )
{
   ...
   ch->race = 0;
   ch->Class = 13;
   ch->speaking = LANG_COLISTANI;
   ch->speaks = LANG_COLISTANI;
   ch->barenumdie = 1;
   ch->baresizedie = 4;
   ch->substate = 0;
   ch->tempnum = 0;
...
void make_wizlist(  )
{
   ...
   unlink( WIZLIST_FILE );
   snprintf( buf, MAX_STRING_LENGTH, " Masters of %s!", sysdata.mud_name );
   towizfile( buf );
      ...
      switch ( word[0] )
      {
         ...
         case 'S':
            if( !str_cmp( word, "Saves" ) )
            {
               ...
               pMobIndex->saving_fortitude = x1;
               pMobIndex->saving_reflex = x2;
               pMobIndex->saving_will = x3;
