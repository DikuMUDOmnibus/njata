...
void do_drink( CHAR_DATA* ch, const char* argument)
{
   ...
   switch ( obj->item_type )
   {
    	  ...
    	  */
            ...
            else
            {
               if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
               {
                  ...
                  {
                     act( AT_ACTION, "$n drinks $p.", ch, obj, NULL, TO_ROOM );
                     if( !hgflag )
                        act( AT_ACTION, "You drink $p.", ch, obj, NULL, TO_CHAR );
                  }
                  else if( obj->in_obj )
                  {
                     act( AT_ACTION, "$n drinks $p from $P.", ch, obj, obj->in_obj, TO_ROOM );
                     if( !hgflag )
                        act( AT_ACTION, "You drink $p from $P.", ch, obj, obj->in_obj, TO_CHAR );
                  }
               }
      ...
      /*
         Removed this bit of code, so you can always drink.
...
/*       Increased amount to 2 to make room for sip command
         amount = 1;
*/
         amount = 2;
...
/* New do_sip function so Dustin can be creepy in bars -Danny */
void do_sip( CHAR_DATA* ch, const char* argument)
{
   ...
   switch ( obj->item_type )
   {
      default:
         if( obj->carried_by == ch )
         {
            act( AT_ACTION, "$n lifts $p up to $s mouth and tries to sip from it...", ch, obj, NULL, TO_ROOM );
            act( AT_ACTION, "You bring $p up to your mouth and try to sip from it...", ch, obj, NULL, TO_CHAR );
         }
         else
         {
            act( AT_ACTION, "$n gets down and tries to sip from $p... (Is $e feeling ok?)", ch, obj, NULL, TO_ROOM );
            act( AT_ACTION, "You get down on the ground and try to sip from $p...", ch, obj, NULL, TO_CHAR );
         }
         break;
      ...
      case ITEM_FOUNTAIN:
      {
         ...
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
            ...
            send_to_char( "You take a small sip from the fountain.\r\n", ch );
         }
         break;
      }
      ...
      case ITEM_DRINK_CON:
      {
         ...
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
            act( AT_ACTION, "$n sips $T from $p.", ch, obj, liq->shortdesc, TO_ROOM );
            act( AT_ACTION, "You sip $T from $p.", ch, obj, liq->shortdesc, TO_CHAR );
         }
          ...
          * gain conditions accordingly              -Nopey
          */
         gain_condition( ch, COND_DRUNK, ( liq->mod[COND_DRUNK] / 2 ) );
         gain_condition( ch, COND_FULL, ( liq->mod[COND_FULL] / 2 ) );
         gain_condition( ch, COND_THIRST, ( liq->mod[COND_THIRST] / 2 ) );
   ...
   if( dest_item == ITEM_CONTAINER )
   {
      ...
      switch ( source->item_type )
      {
         ...
         case ITEM_CORPSE_PC:
            ...
            if( !IS_OBJ_STAT( source, ITEM_CLANCORPSE ) || !IS_SET( ch->pcdata->flags, PCFLAG_PVP ) )
            {
