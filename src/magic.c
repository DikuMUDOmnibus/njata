...
bool is_immune( CHAR_DATA * ch, short damtype )
{
   switch ( damtype )
   {
      ...
      case SD_WATER:
         return ( IS_SET( ch->immune, RIS_WATER ) );
      case SD_AIR:
         return ( IS_SET( ch->immune, RIS_AIR ) );
      case SD_EARTH:
         return ( IS_SET( ch->immune, RIS_EARTH ) );
      case SD_LIFE:
         return ( IS_SET( ch->immune, RIS_LIFE ) );
      case SD_DARKNESS:
         return ( IS_SET( ch->immune, RIS_DARKNESS ) );
      case SD_POISON:
...
int dispel_casting( AFFECT_DATA * paf, CHAR_DATA * ch, CHAR_DATA * victim, int affect, bool dispel )
{
   ...
   if( IS_NPC( ch ) || ch->Class == CLASS_ELEMENTALIST || ch->Class == CLASS_ENCHANTER
                    || ch->Class == CLASS_ILLUSIONIST || ch->Class == CLASS_SCHOLAR )
      is_mage = TRUE;
...
/* Updated to match the modern magic language Solese -Danny */
 ...
 *
 * New and Improved! l = character's level, L = victim's level -Danny
 */
int rd_parse( CHAR_DATA *ch, int level, char *texp, CHAR_DATA *victim, int sn )
{
   unsigned int x;
   ...
   if( len == 1 && isalpha( texp[0] ) )
   {
      switch ( texp[0] )
      {
		...
		case 'Y':
			if( victim )
			{
				return calculate_age( victim );
			}
			else
			{
				bug( "NULL victim: rd_parse: Y\r\n" );
				return 0;
			}
		case 'y':
		...
		case 'k':
		    if( !IS_NPC( ch ) )
		    {
				return ch->pcdata->learned[sn];  /* New variable, training in a skill from 1-100 */
            }
			else
			{
				return level;
    ...
    * work it out
    */
   total = rd_parse( ch, level, sexp[0], victim, sn  );
   switch ( operation )
   {
      case '-':
         total -= rd_parse( ch, level, sexp[1], victim, sn );
         break;
      case '+':
         total += rd_parse( ch, level, sexp[1], victim, sn );
         break;
      ...
      case 'd':
      case 'D':
         total = dice( total, rd_parse( ch, level, sexp[1], victim, sn ) );
         break;
      case 'R':
         total = number_range( total, rd_parse( ch, level, sexp[1], victim, sn ) );
         break;
      case '<':
         total = ( total < rd_parse( ch, level, sexp[1], victim, sn ) );
         break;
      case '>':
         total = ( total > rd_parse( ch, level, sexp[1], victim, sn ) );
         break;
      case '=':
         ...
         break;
      case '{':
         total = UMIN( total, rd_parse( ch, level, sexp[1], victim, sn ) );
         break;
      case '}':
         total = UMAX( total, rd_parse( ch, level, sexp[1], victim, sn ) );
         break;

      case '^':
      {
         unsigned int y = rd_parse( ch, level, sexp[1], victim, sn ), z = total;
...
int dice_parse( CHAR_DATA *ch, int level, const char *texp, CHAR_DATA *victim, int sn )
{
   ...
   return rd_parse( ch, level, buf, victim, sn );
}

/*
 * New Saving Throws - Fortitude, Reflex, and Will -Danny
...
bool saves_fortitude( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level + victim->saving_fortitude ) * 5;
...
bool saves_reflex( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level + victim->saving_reflex ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
...
bool saves_will( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level + victim->saving_will ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
...
void do_cast( CHAR_DATA* ch, const char* argument)
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( !IS_NPC( ch ) )
            manacost = ( skill->min_mana * 100 ) / ( 99 + ch->pcdata->learned[sn] );
         else
            manacost = 0;
         mana = IS_NPC( ch ) ? 0 : UMIN( skill->min_mana, manacost );
...
/*
         if( skill->participants <= 1 )
            break;
*/
         if( skill->type != SKILL_SPELL )  /* Only spells should have the timer */
            break;
      ...
      case 1:
         ...
         if( skill->participants >= 0 ) /* Changed from >1 to >=0 for now */
         {
            int cnt = 1;
            CHAR_DATA *tmp;
            TIMER *t;
...
//            if( cnt >= skill->participants )
            if( cnt >= 1 )
            {
               ...
               dont_wait = TRUE;
               send_to_char( "You finish chanting.\r\n", ch );
               vo = locate_targets( ch, arg2, sn, &victim, &obj );
               if( vo == &pAbort )
                  return;
            }
   ...
   if( !dont_wait && skill->type != SKILL_SPELL ) /* If it's not a spell, use wait_state */
...
/* Old system for failing spells based on percentage die roll
...
ch_ret spell_acid_blast( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   dam = dice( level, 6 );
   if( saves_fortitude( level, victim ) )
      dam /= 2;
...
ch_ret spell_blindness( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_BLIND ) || saves_fortitude( tmp, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
   ...
   for( vch = first_char; vch; vch = trvch_wnext( lc ) )
   {
      ...
      if( vch->in_room == where )
      {
         ...
         if( vch != ch && ( IS_NPC( ch ) ? !IS_NPC( vch ) : IS_NPC( vch ) ) )
            retcode = damage( ch, vch, saves_reflex( level, vch ) ? dam / 2 : dam, sn );
...
ch_ret spell_shrink( int sn, int level, CHAR_DATA * ch, void *vo )
{
   CHAR_DATA *victim = ( CHAR_DATA * ) vo;
   AFFECT_DATA af;
   SKILLTYPE *skill = get_skilltype( sn );
   ...
   do
   {
      af.modifier = number_range( 6, 11 ) - victim->height;
   }
   while( af.modifier == 0 );
   xCLEAR_BITS( af.bitvector );
   affect_to_char( victim, &af );
...
ch_ret spell_charm_person( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_CHARM )
       ...
       || level < victim->level || circle_follow( victim, ch ) || !can_charm( ch ) || saves_will( schance, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
ch_ret spell_control_weather( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   {
      IncreaseWindX( cell, change );
      IncreaseWindY( cell, change );
      UpdateWeather( );
   }
   else if( !str_cmp( target_name, "calmer" ) )
   {
      DecreaseWindX( cell, change );
      DecreaseWindY( cell, change );
      UpdateWeather( );
   }
   else
   {
...
ch_ret spell_curse( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_CURSE ) || saves_will( level, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
ch_ret spell_detect_poison( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( obj->item_type == ITEM_DRINK_CON || obj->item_type == ITEM_FOOD || obj->item_type == ITEM_COOK
         || obj->item_type == ITEM_MEAT || obj->item_type == ITEM_FISH || obj->item_type == ITEM_PLANT )
   {
   ...
   if( IS_NPC( victim ) && !xIS_EMPTY( victim->affected_by ) )
   {
      ...
      if( found ) /* Ok lets see if it is a spell */
      {
         ...
         if( paf )   /*It is a spell lets remove the spell too */
         {
            ...
            if( level < victim->level || saves_will( level, victim ) )
            {
...
ch_ret spell_energy_drain( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   schance = ris_save( victim, victim->level, RIS_DARKNESS );
   if( schance == 1000 || saves_will( schance, victim ) )
   {
...
ch_ret spell_faerie_fog( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   for( ich = ch->in_room->first_person; ich; ich = ich->next_in_room )
   {
      ...
      if( ich == ch || saves_reflex( level, ich ) )
         continue;
...
ch_ret spell_poison( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( schance == 1000 || saves_fortitude( schance, victim ) )
   {
      set_char_color( AT_MAGIC, ch );
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_SLEEP )
       ...
       || level < victim->level
       || ( victim != ch && xIS_SET( victim->in_room->room_flags, ROOM_SAFE ) ) || saves_will( schance, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      if( ch == victim )
         return rSPELL_FAILED;
...
ch_ret spell_summon( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( ( victim = get_char_world( ch, target_name ) ) == NULL
       ...
       || ( IS_NPC( victim ) && saves_will( level, victim ) )
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( !victim->in_room
       ...
       || ( victim != ch && ( saves_will( level, victim ) ) ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
ch_ret spell_weaken( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( is_affected( victim, sn ) || saves_fortitude( level, victim ) )
   {
      send_to_char( "Your magic fails to take hold.\r\n", ch );
      return rSPELL_FAILED;
   }
...
ch_ret spell_acid_breath( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( chance( ch, 2 * level ) && !saves_reflex( level, victim ) )
   {
...
ch_ret spell_possess( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_POSSESS )
       || IS_AFFECTED( ch, AFF_CHARM )
       || level < victim->level || victim->desc || saves_will( ctry, victim ) || !chance( ch, 25 ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
bool check_save( int sn, int level, CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( skill->saves )
      ...
      {
         case SS_FORTITUDE:
            saved = saves_fortitude( level, victim );
            break;
         case SS_REFLEX:
            saved = saves_reflex( level, victim );
            break;
         case SS_WILL:
            saved = saves_will( level, victim );
            break;
      }
   return saved;
}
...
ch_ret spell_attack( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( skill->dice )
      dam = UMAX( 0, dice_parse( ch, level, skill->dice, victim, sn ) );
   else
      dam = dice( 1, level / 2 );
   if( saved )
   {
...
ch_ret spell_area_attack( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   for( vch = ch->in_room->first_person; vch; vch = vch_next )
   {
      ...
      if( saved && SPELL_SAVE( skill ) == SE_NEGATE )
      {
         ...
         dam = dice_parse( ch, level, skill->dice, vch, sn );
      else
         dam = dice( 1, level / 2 );
      if( saved )
      {
   ...
   for( saf = skill->first_affect; saf; saf = saf->next )
   {
      ...
      switch ( saf->bitvector )
      {
         ...
         case AFF_POISON:
            ...
            if( saves_fortitude( schance, victim ) )
            {
               if( SPELL_FLAG( skill, SF_STOPONFAIL ) )
                  return retcode;
               continue;
	  ...
	  af.duration = dice_parse( ch, level, saf->duration, victim, sn );
	  af.modifier = dice_parse( ch, level, saf->modifier, victim, sn );
...
ch_ret spell_affect( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( !groupsp && !areasp )
   {
	  ...
	  if( ( saf = skill->first_affect ) && !saf->next
		&& saf->location == APPLY_STRIPSN && !is_affected( victim, dice_parse( ch, level, saf->modifier, victim, sn ) ) )
      {
         failed_casting( skill, ch, victim, NULL );
         return rSPELL_FAILED;
      }
   ...
   switch ( SPELL_ACTION( skill ) )
   {
      ...
      case SA_CREATE:
         if( SPELL_FLAG( skill, SF_WATER ) ) /* create water */
         {
		    ...
		    water = UMIN( ( skill->dice	? dice_parse( ch, level, skill->dice, NULL, sn ) : level )
         ...
         switch ( SPELL_POWER( skill ) )  /* clone object */
         {
            ...
            case SP_MAJOR:
		       ...
		       clone->timer = skill->dice ? dice_parse( ch, level, skill->dice, NULL, sn ) : 0;
      ...
      case SA_OBSCURE: /* make obj invis */
         ...
         if( IS_OBJ_STAT( obj, ITEM_INVIS )	|| chance( ch, skill->dice ? dice_parse( ch, level, skill->dice, NULL, sn ) : 20 ) )
         {
            failed_casting( skill, ch, NULL, NULL );
            return rSPELL_FAILED;
         }
...
ch_ret spell_create_obj( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   obj->timer = skill->dice ? dice_parse( ch, level, skill->dice, NULL, sn ) : 0;
   successful_casting( skill, ch, NULL, obj );
...
ch_ret spell_create_mob( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   mob->level = UMIN( lvl, skill->dice ? dice_parse( ch, level, skill->dice, NULL, sn ) : mob->level );
